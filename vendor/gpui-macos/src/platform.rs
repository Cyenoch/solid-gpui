        ...
        if let WindowKind::AnchoredPopup(popup) = &options.kind {
            if popup.grab {
                return Err(anyhow::anyhow!(
                    "explicit popup grabs are unavailable on macOS"
                ));
            }
            if MacWindow::native_window(popup.parent).is_none() {
                return Err(anyhow::anyhow!("popup parent window is unavailable"));
            }
        }
