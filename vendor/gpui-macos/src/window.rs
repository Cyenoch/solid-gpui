...
struct MacWindowState {
    ...
    popup: Option<gpui::popup::PopupOptions>,
    popup_requested_size: Size<Pixels>,
...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            let state = Arc::new(Mutex::new(MacWindowState {
                ...
                popup_requested_size: bounds.size,
                popup: if let WindowKind::AnchoredPopup(ref options) = kind {
                    Some(options.clone())
                } else {
                    None
                },
            ...
            match kind {
                ...
                WindowKind::AnchoredPopup(ref options) => {
                    let parent =
                        Self::native_window(options.parent).expect("validated popup parent");
                    ...
                    let _: () = msg_send![native_window, setCollectionBehavior:
                        NSWindowCollectionBehavior::NSWindowCollectionBehaviorFullScreenAuxiliary];
                    let _: () = msg_send![parent, addChildWindow: native_window ordered: NSWindowOrderingMode::NSWindowAbove];
                    native_window.setAcceptsMouseMovedEvents_(YES);
                }
                WindowKind::PopUp => {
            ...
            // Anchored windows resolve their screen position after all generic
            // initialization. The normal origin must never overwrite that result.
            if let WindowKind::AnchoredPopup(ref options) = kind {
                if let Err(error) = window.reposition_popup(options.anchor_rect) {
                    pool.drain();
                    return Err(error);
                }
            } else {
                // Correct the initial origin when the main and primary screens differ.
    ...
    pub fn native_window(handle: AnyWindowHandle) -> Option<id> {
        unsafe {
            let windows: id = msg_send![NSApplication::sharedApplication(nil), windows];
            let count: NSUInteger = msg_send![windows, count];
            for i in 0..count {
                let window: id = msg_send![windows, objectAtIndex: i];
                let known: bool = msg_send![window, isKindOfClass: WINDOW_CLASS];
                let panel: bool = msg_send![window, isKindOfClass: PANEL_CLASS];
                if (known || panel) && get_window_state(&*window).lock().handle == handle {
                    return Some(window);
                }
            }
            None
    ...
    pub fn active_window() -> Option<AnyWindowHandle> {
        unsafe {
            ...
            let main_window: id = msg_send![app, keyWindow];
            if main_window.is_null() {
                return None;
            }
...
impl PlatformWindow for MacWindow {
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        let anchor = {
            ...
            state.popup_requested_size = size;
            state.popup.as_ref().map(|popup| popup.anchor_rect)
        };
    ...
    fn reposition_popup(&mut self, anchor: Bounds<Pixels>) -> anyhow::Result<()> {
        ...
        let parent = Self::native_window(options.parent)
            .ok_or_else(|| anyhow::anyhow!("popup parent window is unavailable"))?;
        unsafe {
            let state = get_window_state(&*parent);
            ...
            let y = if flipped == YES {
                anchor.origin.y.as_f32() as f64
            } else {
                view_bounds.size.height
                    - anchor.origin.y.as_f32() as f64
                    - anchor.size.height.as_f32() as f64
            };
            let rect = NSRect::new(
                NSPoint::new(anchor.origin.x.as_f32() as f64, y),
                NSSize::new(
                    anchor.size.width.as_f32() as f64,
                    anchor.size.height.as_f32() as f64,
                ),
            );
            let in_window: NSRect = msg_send![view, convertRect: rect toView: nil];
            let screen_anchor: NSRect = msg_send![parent, convertRectToScreen: in_window];
            let to_bounds = |rect: NSRect| {
                Bounds::new(
                    point(
            ...
            placement.anchor_rect = to_bounds(screen_anchor);
            let screens = NSScreen::screens(nil);
            let frames = (0..NSArray::count(screens))
                .map(|i| to_bounds(NSScreen::frame(NSArray::objectAtIndex(screens, i))))
                .collect::<Vec<_>>();
            let index = gpui::popup::popup_display(placement.anchor_rect, &frames)
                .ok_or_else(|| anyhow::anyhow!("popup display is unavailable"))?;
            let screen = NSArray::objectAtIndex(screens, index as u64);
            ...
            let frame = NSRect::new(
                NSPoint::new(
                    ...
                    -(bounds.origin.y + bounds.size.height).as_f32() as f64,
                ),
                NSSize::new(
                    bounds.size.width.as_f32() as f64,
                    bounds.size.height.as_f32() as f64,
