...
- **Notification card layout:** a toast is one row — `[icon] [title / message / content] [action] [close]` — with a single 16 px inset on all four sides, 12 px between slots, and no absolutely positioned child. The icon, the action and the close button each sit in a slot one body line tall (at least 24 px), so their glyphs centre on the *first* line of wrapped copy; the copy column stretches to the row, so its own inset stays symmetric. The close button is always painted in the muted foreground (it used to be `invisible()` until the card was hovered, which left the toast with no visible way out) and carries the `notification-card`, `notification-icon`, `notification-copy`, `notification-action`, `notification-close` and `notification-close-button` test-only selectors.
