use gpui::{
    Anchor, App, Entity, IntoElement, ParentElement, Pixels, Point, RenderOnce, Styled, Window,
...
impl FpsOverlay {
    pub fn new(monitor: &Entity<FpsMonitor>) -> Self {
        Self {
            ...
            frame_budget: None,
            offset: gpui::point(MARGIN, MARGIN),
        }
    }

    /// Insets from the horizontal and vertical edges of the containing window.
    ...
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .map(|this| match self.anchor {
                Anchor::TopLeft => this.top(offset.y).left(offset.x),
                Anchor::TopRight => this.top(offset.y).right(offset.x),
                Anchor::BottomLeft => this.bottom(offset.y).left(offset.x),
                Anchor::BottomRight => this.bottom(offset.y).right(offset.x),
                Anchor::TopCenter => this.top(offset.y).left_0().right_0().justify_center(),
                Anchor::BottomCenter => this.bottom(offset.y).left_0().right_0().justify_center(),
                Anchor::LeftCenter => this.left(offset.x).top_0().bottom_0().items_center(),
                Anchor::RightCenter => this.right(offset.x).top_0().bottom_0().items_center(),
            })
            .child(self.monitor)
    }
}
