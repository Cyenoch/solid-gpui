...
impl DatePickerState {
    ...
    pub fn set_date_format(&mut self, format: impl Into<SharedString>, cx: &mut Context<Self>) {
        self.date_format = format.into();
        cx.notify();
    }
    pub fn set_first_day_of_week(&mut self, day: Weekday, cx: &mut Context<Self>) {
        self.first_day_of_week = day;
        cx.notify();
    }
    pub fn set_disabled_matcher_shared(
        &mut self,
        matcher: Option<Rc<Matcher>>,
        cx: &mut Context<Self>,
    ) {
        self.disabled_matcher = matcher.clone();
    ...
    fn update_date(
        ...
        let accepted = self.calendar.update(cx, |view, cx| {
            if view.apply_date(date) {
                cx.notify();
                true
            } else {
                false
