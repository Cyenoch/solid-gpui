...
use crate::{
    ActiveTheme as _, ComponentChild, Disableable, FocusableExt as _, Icon, Selectable, Sizable,
...
pub struct InputGroup {
    ...
    control: Option<ComponentChild<Input>>,
...
impl InputGroup {
    ...
    pub fn input(mut self, input: impl Into<InputGroupControl>) -> Self {
        self.control = Some(ComponentChild::from(input.into().0));
        self
    }

    /// Set the control and attach the embedding host's render boundary. The
    /// group restyles the native control first; the boundary wraps the
    /// restyled element, so host configuration survives the group's frame.
    pub fn input_with_boundary(
        mut self,
        input: impl Into<InputGroupControl>,
        boundary: Rc<dyn Fn(AnyElement) -> AnyElement>,
    ) -> Self {
        self.control = Some(ComponentChild::with_boundary(input.into().0, boundary));
        self
    }
...
impl RenderOnce for InputGroup {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let control = self
            .control
            .map(|input| input.render_with(|input| render_control(input, presentation, multiline)));
        let theme = cx.theme();
...
pub struct InputGroupAddon {
    ...
    style: StyleRefinement,
    children: Vec<ComponentChild<AnyElement>>,
}
...
impl InputGroupAddon {
    ...
    /// Append a child wrapped in the embedding host's render boundary. The
    /// group's native scan (inherited disabled state) sees the element before
    /// its boundary, and the boundary wraps the styled child.
    pub fn child_with_boundary(
        mut self,
        element: impl Into<AnyElement>,
        boundary: Rc<dyn Fn(AnyElement) -> AnyElement>,
    ) -> Self {
        self.children
            .push(ComponentChild::with_boundary(element.into(), boundary));
        self
    }
    ...
    fn render_in_group(
        ...
        for child in &mut self.children {
            if let Some(button) =
                button_element::InputGroupButtonElement::from_element(&mut **child)
            {
        ...
        h_flex()
            ...
            .children(
                self.children
                    .into_iter()
                    .map(|child| child.render_with(addon_child)),
            )
            .into_any_element()
    }
}
...
impl ParentElement for InputGroupAddon {
    fn extend(&mut self, elements: impl IntoIterator<Item = AnyElement>) {
        self.children
            .extend(elements.into_iter().map(ComponentChild::from));
    }
}
