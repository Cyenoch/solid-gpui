...
impl DialogButtonProps {
    /// The standard action footer, using native localization and dialog actions.
    pub fn render_footer(&self) -> super::DialogFooter {
        super::DialogFooter::new()
            .when(self.is_cancel_shown(), |footer| {
                footer.child(self.render_cancel())
            })
            .child(self.render_ok())
    }
    ...
    pub(crate) fn render_ok(&self) -> AnyElement {
        let ok_text = self
            .ok_text
            .clone()
    ...
    pub(crate) fn render_cancel(&self) -> AnyElement {
...
impl RenderOnce for Dialog {
    fn render(mut self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // scrolling to the body region below. The floor keeps a very short
        // window from collapsing the popup below its own chrome.
