    ...
    Y: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        // An identified chart keeps its stroke tessellated across frames;
        // without an id, sibling charts would share one cache slot and thrash
        // it. The stroke is tessellated once and reused while the projected
        ...
        // One line per chart: dropping past the first slot keeps the cache
        // bounded when a rebuilt chart paints fewer shapes.
        if let Some(caches) = self
            .id
            .is_some()
            .then(|| PathCaches::for_paint("line-chart", window, cx))
        {
