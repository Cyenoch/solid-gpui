...
impl<T> Plot for SankeyChart<T> {
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        // Links first, under the nodes. The links of the hovered node keep
        // their opacity while the rest fade behind them. Each ribbon is
        // tessellated once and reused while its layout geometry — ribbon
        // centers, end positions and end widths — stays the same; each frame
        // only moves it to the plot's origin. Link i keeps the cache in slot
        // i.
