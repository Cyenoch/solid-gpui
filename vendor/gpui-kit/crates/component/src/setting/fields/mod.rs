...
pub trait AnySettingField {
    fn render_boundary(&self, element: gpui::AnyElement) -> gpui::AnyElement {
        element
    }
...
impl<F: AnySettingField> AnySettingField for crate::ComponentChild<F> {
    fn render_boundary(&self, element: gpui::AnyElement) -> gpui::AnyElement {
        self.wrap_element(element)
    }
    fn as_any(&self) -> &dyn std::any::Any {
        (**self).as_any()
    }
