...
/// The fade duration the compatibility theme applies to streamed-in text.
/// Public so motion-aware hosts reuse the policy without hardcoding it.
pub const STREAM_FADE: Duration = Duration::from_millis(350);
...
mod tests {
    ...
    impl Render for StatelessMarkdown {
        fn render(&mut self, _: &mut gpui::Window, _: &mut Context<Self>) -> impl IntoElement {
            ...
            div().child(
                super::markdown(include_str!("test-fixture.md")).markdown_block_parser(|_, _| None),
            )
        }
    }
