...
pub struct AppMenuBar {
    revision: u64,
    _menus_subscription: Subscription,
...
impl AppMenuBar {
    ...
    pub fn new(cx: &mut App) -> Entity<Self> {
        cx.new(|cx| {
            let mut this = Self {
                revision: 0,
                _menus_subscription: cx.observe_global::<GlobalState>(|view: &mut Self, cx| {
                    if view.revision != GlobalState::global(cx).app_menus_revision() {
                        view.reload(cx);
                    }
                }),
                selected_index: None,
    ...
    pub fn reload(&mut self, cx: &mut Context<Self>) {
        self.revision = GlobalState::global(cx).app_menus_revision();
...
mod tests {
    ...
    #[gpui::test]
    fn follows_application_menu_revisions_without_reloading_for_other_global_changes(
        cx: &mut TestAppContext,
    ) {
        cx.update(crate::init);
        let bar = cx.update(AppMenuBar::new);
        cx.run_until_parked();
        cx.update(|cx| {
            cx.update_global::<GlobalState, _>(|state, _| {
                state.set_app_menus(vec![gpui::Menu::new("File").owned()])
            })
        });
        cx.run_until_parked();
        let first = bar.read_with(cx, |bar, _| {
            assert_eq!(bar.menus.len(), 1);
            bar.menus[0].entity_id()
        });
        cx.update(|cx| cx.update_global::<GlobalState, _>(|_, _| {}));
        cx.run_until_parked();
        assert_eq!(bar.read_with(cx, |bar, _| bar.menus[0].entity_id()), first);
        cx.update(|cx| {
            cx.update_global::<GlobalState, _>(|state, _| {
                state.set_app_menus(vec![
                    gpui::Menu::new("File").owned(),
                    gpui::Menu::new("Edit").owned(),
                ])
            })
        });
        cx.run_until_parked();
        assert_eq!(bar.read_with(cx, |bar, _| bar.menus.len()), 2);
    }
}
