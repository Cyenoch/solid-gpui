...
struct TableLocation {
    ...
    row_key: Option<SharedString>,
    column_key: Option<SharedString>,
}
    ...
    D: TableDelegate,
{
    ...
    /// Synchronize selection without interaction events or scroll movement.
    pub fn project_selection(
        ...
        self.selected_row = row.filter(|row| *row < self.delegate.rows_count(cx));
        self.selected_col = column.filter(|column| *column < self.col_groups.len());
        self.selected_cell =
            cell.filter(|(r, c)| *r < self.delegate.rows_count(cx) && *c < self.col_groups.len());
        self.selection_mode = if self.selected_cell.is_some() {
            SelectionMode::Cell
        } else if self.selected_col.is_some() {
    ...
    pub fn column_widths(&self) -> Vec<(SharedString, Pixels)> {
        self.col_groups
            .iter()
            .map(|g| (g.column.key.clone(), g.width))
            .collect()
    }
    /// Apply a programmatic width without emitting a resize interaction.
    pub fn set_column_width(&mut self, key: &str, width: Pixels, cx: &mut Context<Self>) -> bool {
        let Some(column) = self
            .col_groups
            .iter_mut()
            .find(|c| c.column.key.as_ref() == key)
        else {
            return false;
        };
        if width < column.column.min_width
            || width > column.column.max_width
            || !width.as_f32().is_finite()
        {
            return false;
        }
        column.width = width;
    ...
    fn emit_event(&mut self, event: TableEvent, cx: &mut Context<Self>) {
        self.delegate.on_event(&event, cx);
        cx.emit(event);
    }
    fn location(&self, row: Option<usize>, column: Option<usize>, cx: &gpui::App) -> TableLocation {
        TableLocation {
            row_index: row,
            row_key: row.and_then(|r| self.delegate.row_key(r, cx)),
            column_key: column
                .and_then(|c| self.col_groups.get(c))
                .map(|c| c.column.key.clone()),
        }
    }
    fn resolve_row(&self, location: &TableLocation, cx: &gpui::App) -> Option<usize> {
        let row = match &location.row_key {
            Some(key) => self.delegate.row_index(key, cx),
            None => location.row_index,
        }?;
        (row < self.delegate.rows_count(cx)).then_some(row)
    }
    fn resolve_column(&self, location: &TableLocation) -> Option<usize> {
        location
            .column_key
            .as_ref()
            .and_then(|key| self.col_groups.iter().position(|g| &g.column.key == key))
    }
    fn resolve_cell(&self, location: &TableLocation, cx: &gpui::App) -> Option<(usize, usize)> {
        Some((
            self.resolve_row(location, cx)?,
            self.resolve_column(location)?,
        ))
    }
    ...
    pub fn set_selected_row(&mut self, row_ix: usize, cx: &mut Context<Self>) {
        ...
        self.emit_event(TableEvent::SelectRow(row_ix), cx);
    ...
    pub fn set_selected_col(&mut self, col_ix: usize, cx: &mut Context<Self>) {
        ...
        self.emit_event(TableEvent::SelectColumn(col_ix), cx);
        cx.notify();
    }
    ...
    pub fn set_selected_cell(&mut self, row_ix: usize, col_ix: usize, cx: &mut Context<Self>) {
        ...
        self.emit_event(TableEvent::SelectCell(row_ix, col_ix), cx);
        cx.notify();
    }
    ...
    fn prepare_col_groups(&mut self, cx: &mut Context<Self>) {
        let old: std::collections::HashMap<_, _> = std::mem::take(&mut self.col_groups)
            .into_iter()
            .map(|g| (g.column.key.clone(), g))
            .collect();
        self.col_groups = (0..self.delegate.columns_count(cx))
            .map(|col_ix| {
                ...
                let width = old
                    .get(&column.key)
                    .filter(|old| old.column.width == column.width)
                    .map_or(column.width, |old| old.width)
    ...
    fn on_row_right_click(
        ...
        self.emit_event(TableEvent::RightClickedRow(row_ix), cx);
    }
    ...
    fn on_cell_right_click(
        ...
        self.right_clicked_row = None;
        self.emit_event(TableEvent::RightClickedCell(row_ix, col_ix), cx);
    }
    ...
    fn on_row_left_click(
        ...
        if e.click_count() == 2 {
            self.emit_event(TableEvent::DoubleClickedRow(row_ix), cx);
        }
    }
    ...
    fn on_cell_click(
        ...
        if is_double_click {
            self.emit_event(TableEvent::DoubleClickedCell(row_ix, col_ix), cx);
        }
    }
    ...
    fn move_column(
        ...
        let selected_row = self.selected_row.map(|r| self.location(Some(r), None, cx));
        let selected_col = self.selected_col.map(|c| self.location(None, Some(c), cx));
        let selected_cell = self
            .selected_cell
            .map(|(r, c)| self.location(Some(r), Some(c), cx));
        ...
        self.selected_row = selected_row.as_ref().and_then(|p| self.resolve_row(p, cx));
        self.selected_col = selected_col.as_ref().and_then(|p| self.resolve_column(p));
        self.selected_cell = selected_cell
            .as_ref()
            .and_then(|p| self.resolve_cell(p, cx));
        self.update_header_layout(cx);

        self.emit_event(TableEvent::MoveColumn(col_ix, to_ix), cx);
        cx.notify();
    }
    ...
    fn render_row_header_cell(
        ...
        let location = self.location(Some(row_ix), None, cx);
        div()
            ...
            .when(!is_head, |this| {
                this.when(self.row_selectable, |this| {
                    this.on_click(cx.listener(move |table, _, _window, cx| {
                        if let Some(row_ix) = table.resolve_row(&location, cx) {
    ...
    fn render_sort_icon(
        ...
        let location = self.location(None, Some(col_ix), cx);
        ...
        Some(
            div()
                ...
                .on_click(cx.listener(move |table, _, window, cx| {
                    if let Some(col_ix) = table.resolve_column(&location) {
            ...
            .child(
                self.render_cell(None, col_ix, window, cx)
                    ...
                    .on_click(cx.listener(move |this, _, window, cx| {
                        if let Some(col_ix) = this.resolve_column(&location) {
    ...
    fn render_table_row(
        ...
        if row_ix < rows_count {
            ...
            let location = self.location(Some(row_ix), None, cx);
            let right_location = location.clone();
            ...
            tr.test_support()
                                                    ...
                                                    .when(self.cell_selectable, |this| {
                                                        ...
                                                        this.on_click(cx.listener(
                                                            move |table, e, window, cx| {
                                                                let Some((row_ix, col_ix)) = table
                                                                    .resolve_cell(&location, cx)
                                                                else {
                                                                    return;
                                                                };
                                                                table.on_cell_click(
                ...
                .on_mouse_down(
                    ...
                    cx.listener(move |this, e, window, cx| {
                        if let Some(row_ix) = this.resolve_row(&right_location, cx) {
                            this.on_row_right_click(e, Some(row_ix), window, cx);
                        }
                    }),
                )
                .on_click(cx.listener(move |this, e, window, cx| {
                    if let Some(row_ix) = this.resolve_row(&location, cx) {
