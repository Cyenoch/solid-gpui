...
use crate::{
    ...
    menu::PopupMenu,
    table::{Column, ColumnGroup, ColumnSort, TableEvent, TableState, loading::Loading},
};
...
pub trait TableDelegate: Sized + 'static {
    ...
    /// Optional stable row identities for interaction reconciliation across model updates.
    fn row_key(&self, row_ix: usize, cx: &App) -> Option<SharedString> {
        None
    }
    fn row_index(&self, key: &str, cx: &App) -> Option<usize> {
        None
    }
    /// Receives the interaction synchronously against the model that produced it.
    fn on_event(&mut self, event: &TableEvent, cx: &mut Context<TableState<Self>>) {}
