...
pub(crate) struct SearchableListAdapter<D: SearchableListDelegate + 'static> {
    ...
    on_confirm: Box<
        dyn Fn(Option<(IndexPath, D::Item)>, bool, &mut Window, &mut Context<ListState<Self>>)
            + 'static,
    >,
...
impl<D: SearchableListDelegate + 'static> SearchableListAdapter<D> {
    pub(crate) fn new(
        delegate: D,
        on_confirm: impl Fn(
            Option<(IndexPath, D::Item)>,
            bool,
            &mut Window,
            &mut Context<ListState<Self>>,
        ) + 'static,
...
impl<D: SearchableListDelegate + 'static> ListDelegate for SearchableListAdapter<D> {
    ...
    fn can_select_index(&self, ix: IndexPath, cx: &App) -> bool {
        self.delegate
            .item(ix)
            .is_some_and(|item| self.delegate.is_item_enabled(ix, item, cx))
    }
    ...
    fn confirm(&mut self, secondary: bool, window: &mut Window, cx: &mut Context<ListState<Self>>) {
        let selected = self
            .selected_index
            .and_then(|ix| self.delegate.item(ix).cloned().map(|item| (ix, item)));
        (self.on_confirm)(selected, secondary, window, cx);
    }
