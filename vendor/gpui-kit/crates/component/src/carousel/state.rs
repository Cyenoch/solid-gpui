...
impl CarouselState {
    ...
    pub fn set_item_count(&mut self, item_count: usize, cx: &mut Context<Self>) {
        ...
        self.reconcile_items(item_count, selection, cx);
    }

    /// Reconcile a changed item order, cancelling geometry and gestures from the old order.
    /// The caller supplies the new index of its retained logical selection.
    pub fn reconcile_items(
        ...
        self.selected_index = (item_count > 0).then(|| selection.unwrap_or(0).min(item_count - 1));
