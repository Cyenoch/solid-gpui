...
impl NotificationList {
    /// Find one notification using the same owner key as `Notification::id1`.
    pub fn notification1<T: 'static>(
        &self,
        key: impl Into<ElementId>,
    ) -> Option<Entity<Notification>> {
        self.notifications
            .get(&(TypeId::of::<T>(), key.into()).into())
            .cloned()
    }

    /// Update a live in-app toast in place. Position, transition, subscriptions
    /// and remaining auto-hide time are retained. A retired toast stays retired.
    pub fn revise(
        &mut self,
        ...
        let Some(current) = self.notifications.get(&id).cloned() else {
            return false;
        };
        let previous = current.read(cx);
        if previous.transition_status == ToastTransitionStatus::Ending {
            return false;
        }
        let timeout_changed = previous.autohide != notification.autohide;
        notification.transition_status = previous.transition_status;
        if notification
            .delivery
        ...
        if timeout_changed {
            self.notifications
                .set_timeout(&id, notification.autohide.then_some(Duration::from_secs(5)));
        }
        current.update(cx, |current, cx| {
