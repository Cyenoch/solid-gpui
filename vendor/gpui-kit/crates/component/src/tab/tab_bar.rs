...
pub struct TabBar {
    ...
    children: SmallVec<[ComponentChild<Tab>; 2]>,
...
impl TabBar {
    ...
    pub fn children(
        mut self,
        children: impl IntoIterator<Item = impl Into<ComponentChild<Tab>>>,
    ) -> Self {
    ...
    pub fn child(mut self, child: impl Into<ComponentChild<Tab>>) -> Self {
...
impl RenderOnce for TabBar {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for (ix, child) in self.children.into_iter().enumerate() {
            ...
            let mut tab = child.map_native(|child| {
                let mut tab = child
                    .ix(ix)
