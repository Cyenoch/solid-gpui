...
impl MessageScrollerState {
    ...
    /// Logical reading anchor, including the offset within a variable-height row.
    pub fn anchor(&self) -> ListOffset {
        self.list_state.logical_scroll_top()
    }

    /// Restore a surviving caller-owned row after a keyed model reconciliation.
    pub fn restore_anchor(&mut self, anchor: ListOffset, cx: &mut Context<Self>) {
