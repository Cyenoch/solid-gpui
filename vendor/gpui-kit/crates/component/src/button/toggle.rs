...
pub struct ToggleGroup {
    ...
    disabled: bool,
    segmented: bool,
    items: Vec<ComponentChild<Toggle>>,
...
impl ToggleGroup {
    ...
    pub fn child(mut self, toggle: impl Into<ComponentChild<Toggle>>) -> Self {
    ...
    pub fn children(
        mut self,
        children: impl IntoIterator<Item = impl Into<ComponentChild<Toggle>>>,
    ) -> Self {
...
impl RenderOnce for ToggleGroup {
    fn render(self, _: &mut Window, _: &mut App) -> impl IntoElement {
        ...
        let on_click: Option<Rc<dyn Fn(&Vec<bool>, &mut Window, &mut App)>> =
            self.on_click.map(Rc::from);

        BaseToggleGroup::new(self.id).axis(Axis::Horizontal).child(
            crate::h_flex()
                .items_center()
                        ...
                        move |(ix, item)| {
                            item.map_native(|item| {
                                ...
                                item.disabled(effective_disabled)
                                    ...
                                    .when_some(on_click, |item, on_click| {
                                        item.on_click(move |_, window, cx| {
                                            let mut next = checks.as_ref().clone();
                                            next[ix] = !next[ix];
...
mod tests {
    ...
    #[gpui::test]
    fn toggle_group_preserves_child_callback_without_group_callback(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn toggle_group_pointer_flips_only_the_clicked_rendered_value(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn toggle_group_keyboard_click_reaches_the_group_callback(cx: &mut TestAppContext) {
        let (cx, child_clicks, changes) = toggle_group_harness(cx, true);
        cx.update(|window, cx| window.focus_next(cx));
        activate_key(cx, "enter");
        assert_eq!(child_clicks.get(), 0);
