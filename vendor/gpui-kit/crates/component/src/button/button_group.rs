...
use gpui::{RenderOnce, StyleRefinement, Styled, div, prelude::FluentBuilder as _};
use std::rc::Rc;
...
pub struct ButtonGroup {
    ...
    children: Vec<ComponentChild<Button>>,
...
impl ButtonGroup {
    ...
    pub fn child(mut self, child: impl Into<ComponentChild<Button>>) -> Self {
    ...
    pub fn children(
        mut self,
        children: impl IntoIterator<Item = impl Into<ComponentChild<Button>>>,
    ) -> Self {
...
impl RenderOnce for ButtonGroup {
    fn render(self, _: &mut Window, _: &mut App) -> impl IntoElement {
        ...
        let selected_ixs = Rc::new(selected_ixs);
                ...
                self.children
                    ...
                    .map(|(child_index, child)| {
                        child.map_native(|child| {
                            let selected_ixs = selected_ixs.clone();
                            let on_click = self.on_click.clone();
                            let disabled = self.disabled || child.disabled;
                            // The group as a whole is a toggle control, so every
                            // child advertises its pressed state.
                            let selected = child.selected;
                            let child = child.toggled(selected).disabled(disabled);
                            ...
                            .when_some(on_click, |this, on_click| {
                                this.on_click(move |_, window, cx| {
                                    let mut next = selected_ixs.as_ref().clone();
                                    if self.multiple {
                                        if let Some(pos) =
                                            next.iter().position(|i| *i == child_index)
                                        {
                                            next.remove(pos);
                                        } else {
                                            next.push(child_index);
                                        }
                                    } else {
                                        next.clear();
                                        next.push(child_index);
...
mod tests {
    ...
    #[gpui::test]
    fn group_callback_overrides_the_child_callback(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn child_callback_survives_without_a_group_callback(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn single_and_multiple_results_use_the_rendered_selection(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn keyboard_click_reaches_the_group_callback(cx: &mut TestAppContext) {
    ...
    #[gpui::test]
    fn disabled_state_is_independent_of_builder_order(cx: &mut TestAppContext) {
