...
impl TextViewState {
    ...
    fn new(format: TextViewFormat, text: &str, cx: &mut Context<Self>) -> Self {
        ...
        let _parse_task = cx.background_spawn(UpdateFuture::new(rx, tx_result));
    ...
    /// Replace content and parser together on the same retained entity.
    pub fn set_content(&mut self, format: TextViewFormat, text: &str, cx: &mut Context<Self>) {
...
impl UpdateFuture {
    fn new(rx: Receiver<UpdateOptions>, tx_result: Sender<ParsedUpdate>) -> Self {
        Self {
        ...
        loop {
            match self.rx.as_mut().poll_next(cx) {
                Poll::Ready(Some(mut options)) => {
                    ...
                    let res = parse_content(options.format, self.content.clone(), &options);
...
mod tests {
    ...
    fn update_options_merge_keeps_latest_full_text() {
        let mut options = UpdateOptions {
            format: TextViewFormat::Markdown,
            revision: 1,
    ...
    fn update_future_yields_before_coalescing_all_queued_updates() {
        ...
        let mut future = Box::pin(UpdateFuture::new(rx, tx_result));
