    ...
    T: Interpolate + PartialEq + 'static,
{
    ...
    /// Samples the sequence without consuming it, so a caller can retain the
    /// built sequence across frames instead of rebuilding its steps each one.
    pub fn sample_ref(&self, window: &mut Window, cx: &mut App) -> SequenceSample<T> {
        self.sample_impl(window, cx)
    }
    ...
    /// The shared sampling body; it never needs ownership of the sequence.
    fn sample_impl(&self, window: &mut Window, cx: &mut App) -> SequenceSample<T> {
        ...
        let id = ElementId::NamedChild(self.id.0.clone().into(), "__sequence".into());
