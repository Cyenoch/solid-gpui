...
//!
//! A host that owns the flag from the start — before `init` runs — can also
//! switch Base's system reading off entirely with
//! [`set_system_reduce_motion_enabled(false, ..)`]; no platform probe runs and
//! no change subscription is kept. Base reads the OS preference only where an
//! application has not taken it over this way.
//!
...
struct SystemReduceMotion {
    applied: Option<bool>,
    following: bool,
    /// Set by the embedding host through
    /// [`set_system_reduce_motion_enabled`]: when true, Base never reads the
    /// platform preference, never subscribes to it, and never writes the
    /// flag — the application owns the flag outright.
    disabled: bool,
}

impl Global for SystemReduceMotion {}

/// Enables or disables Base's reading of the operating system's
/// reduced-motion preference.
///
/// A host that manages the flag itself calls this with `false` before
/// `gpui_base::init` (or `gpui_component::init`): from then on Base never
/// consults the platform, never subscribes to its change signal — including
/// a subscription an earlier reading already started — and never writes
/// [`App::set_reduce_motion`]. Passing `true` hands ownership back to Base
/// and applies the current preference again. The default is enabled, so
/// applications that never call this keep upstream behavior.
pub fn set_system_reduce_motion_enabled(enabled: bool, cx: &mut App) {
    cx.default_global::<SystemReduceMotion>().disabled = !enabled;
    if enabled {
...
mod tests {
    ...
    use super::{apply_preference, apply_system_reduce_motion, set_system_reduce_motion_enabled};
    ...
    #[gpui::test]
    fn a_disabled_system_reading_never_writes_the_flag(cx: &mut TestAppContext) {
        cx.update(|cx| {
            set_system_reduce_motion_enabled(false, cx);
            apply_preference(Some(true), cx);
            assert!(!cx.reduce_motion());
            apply_system_reduce_motion(cx);
            assert!(!cx.reduce_motion());

            set_system_reduce_motion_enabled(true, cx);
            // Under the test scheduler the platform is not consulted even
            // after re-enabling, so the flag stays where the application set
            // it.
            assert!(!cx.reduce_motion());
        });
    }
