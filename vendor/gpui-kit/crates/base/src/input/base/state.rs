...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    /// Current selections, with the active cursor first and direction preserved.
    pub fn cursor_selections(&self) -> impl Iterator<Item = super::cursor::DirectedSelection> + '_ {
        self.selections
            .iter()
            .map(|selection| super::cursor::DirectedSelection {
                anchor: if selection.reversed {
                    selection.end
                } else {
                    selection.start
                },
                head: selection.cursor_offset(),
            })
    }

    /// Atomically replace directed UTF-8 selections, merging overlapping ranges.
    /// Invalid offsets or active IME composition leave all native state untouched.
    pub fn set_cursor_selections(
        &mut self,
        selections: &[super::cursor::DirectedSelection],
        cx: &mut Context<Self>,
    ) -> Result<(), String> {
        if selections.is_empty() {
            return Err("at least one selection is required".into());
        }
        if self.ime_marked_range.is_some() {
            return Err("selection cannot change during IME composition".into());
        }
        if selections.len() > 1 && !matches!(self.mode, LayoutMode::CodeEditor { .. }) {
            return Err("multiple selections require an editor".into());
        }
        if selections.iter().any(|selection| {
            [selection.anchor, selection.head].iter().any(|offset| {
                *offset > self.text.len() || self.cursor_boundary(*offset, Bias::Left) != *offset
            })
        }) {
            return Err("selections must use UTF-8 boundaries without splitting CRLF".into());
        }
        let ranges = selections
            .iter()
            .map(|selection| {
                let id = self.selections.generate_id();
                let mut range = CursorSelection::new(
                    id,
                    selection.anchor.min(selection.head),
                    selection.anchor.max(selection.head),
                );
                range.reversed = selection.head < selection.anchor;
                range
            })
            .collect();
        self.undo_manager.break_transaction_coalescing();
        self.selections.replace_all(ranges);
        self.selections.merge_overlapping();
        self.selected_word_range = None;
    ...
    pub(super) fn next_boundary(&self, offset: usize) -> usize {
        ...
        let offset = self.cursor_boundary(offset, Bias::Right);
...
impl InputBaseState<crate::input::TextareaMode> {
    ...
    /// Switch to fixed-row layout without replacing editing state.
    pub fn set_rows(&mut self, rows: usize, cx: &mut Context<Self>) {
        match &mut self.mode {
            LayoutMode::PlainText { rows: value, .. } => *value = rows,
            _ => {
                self.mode = LayoutMode::PlainText {
                    tab: crate::input::TabSize::default(),
                    rows,
                }
            }
        }
