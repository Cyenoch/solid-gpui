...
///
/// ResizableState owns keyed reconciliation. This snapshot detects which tree
/// edits authorize adoption of persisted proportions.
struct CachedSplit {
...
impl DockArea {
    ...
    /// Refresh panel metadata/chrome after the host changes an existing panel.
    /// The layout tree and retained container identities stay intact.
    pub fn refresh_panels(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        self.reconcile(window, cx);
        for cached in self.groups.values() {
            cached.entity.update(cx, |_, cx| cx.notify());
        }
        for cached in self.tiles.values() {
            cached.entity.update(cx, |_, cx| cx.notify());
        }
        cx.notify();
    }

    /// The retained tab group for a live layout node.
    pub fn tab_group(&self, node: NodeId) -> Option<Entity<TabGroup>> {
        self.groups.get(&node).map(|cached| cached.entity.clone())
    }

    /// The retained tiles canvas for a live layout node; exposes its native history and interactions.
    pub fn tiles_state(&self, node: NodeId) -> Option<Entity<TilesState>> {
        self.tiles.get(&node).map(|cached| cached.entity.clone())
    }
...
impl DockArea {
    ...
    pub fn move_panel(
        ...
        // Removing a source group can collapse a split. Preserve the current
        // measured shares of its surviving siblings before editing the tree.
        if let Some(source) = source {
            self.adopt_measured_sizes(source, cx);
        }
                ...
                ContainerPlan::Split {
                    ...
                    if previous != children || adopted != sizes {
                        state.update(cx, |state, cx| {
                            let keys = children
                                .iter()
                                .map(|id| gpui::ElementId::from(("dock-pane", id.as_u64())))
                                .collect::<Vec<_>>();
                            state.sync_panels(axis, &keys, cx);
        ...
        match node.kind() {
            PaneRef::Split {
                ...
                let panels: Vec<_> = children
                    ...
                    .enumerate()
                    .map(|(ix, (child, size))| {
                        resizable_panel(("dock-pane", child.id().as_u64()))
                            .visible(shown[ix])
...
mod tests {
    ...
    fn an_all_hidden_container_reports_itself_invisible(cx: &mut TestAppContext) {
        ...
        // it. `render_node` feeds this straight to `resizable_panel(("dock-pane", child.id().as_u64())).visible`.
        let (area, cx) = setup(cx);
        let beta = cx.update(|window, cx| {
            let alpha = TestPanel::new("Alpha", cx);
            let beta = TestPanel::new("Beta", cx);
