...
impl TilesState {
    ...
    /// Set one tile's geometry as a completed edit, using the same history and
    /// notification path as a move/resize gesture. An in-flight gesture is not overwritten.
    pub fn set_bounds(
        &mut self,
        panel: PanelId,
        bounds: Bounds<Pixels>,
        ...
        if self.moving.is_some() || self.resizing.is_some() {
            return false;
        }
        let Some(previous) = self.bounds_of(panel) else {
            return false;
        };
        self.apply_bounds(panel, bounds, cx);
        self.record(panel, previous, cx);
        true
    }
