...
impl OtpState {
    ...
    /// Preserve focus and subscriptions while changing the code length.
    /// A shorter code retains its leading characters; no user edit is emitted.
    pub fn set_length(&mut self, length: usize, cx: &mut Context<Self>) {
        self.length = length;
        self.value = self.value.chars().take(length).collect::<String>().into();
        cx.notify();
    }
