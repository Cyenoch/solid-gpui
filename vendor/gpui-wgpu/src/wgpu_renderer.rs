...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        // wgpu 29's browser backend advertises only Opaque, while configure()
        // explicitly supports PreMultiplied. Use that supported WebGPU mode.
        let transparent_alpha_mode =
            if context.adapter.get_info().backend == wgpu::Backend::BrowserWebGpu {
