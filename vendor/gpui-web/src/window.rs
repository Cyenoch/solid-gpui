        ...
        Closure::new(move |entries: js_sys::Array| {
            ...
            let (physical_width, physical_height, logical_width, logical_height) =
                if inner.has_device_pixel_support {
                    ...
                    // contentRect is authoritative for CSS layout. Browser device
                    // emulation can report physical pixels at a different ratio
                    // from devicePixelRatio; dividing by that ratio shrinks the UI.
            ...
            let effective_scale = if logical_width > 0.0 {
                physical_width as f64 / logical_width as f64
            } else {
                dpr
            };
            let dpr_f32 = effective_scale as f32;
            let scale_changed = inner.notify_scale.replace(false)
                || (inner.state.borrow().scale_factor - dpr_f32).abs() > f32::EPSILON;
            ...
            let (logical_width, logical_height) =
                if (clamped_width, clamped_height) != (physical_width, physical_height) {
                    (
                        (clamped_width as f64 / effective_scale) as f32,
                        (clamped_height as f64 / effective_scale) as f32,
                    )
                } else {
...
impl PlatformWindow for WebWindow {
    ...
    fn background_appearance(&self) -> WindowBackgroundAppearance {
        self.inner.state.borrow().background
    }
