...
pub enum CalendarEvent {
    /// A user activation changed the value, including an incomplete range.
    Change(Date),
    Selected(Date),
}
...
impl CalendarState {
    ...
    pub fn activate_date(&mut self, value: NaiveDate, cx: &mut Context<Self>) -> bool {
        ...
        if self.date() != previous {
            cx.emit(CalendarEvent::Change(self.date()));
        }
        if complete {
    ...
    /// Move the visible page without changing the selection.
    pub fn show_date(&mut self, date: NaiveDate, cx: &mut Context<Self>) {
    ...
    impl EventHarness {
        fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
            ...
            this._subscription = Some(cx.subscribe(&calendar, move |_, _, event, _| {
                let CalendarEvent::Selected(date) = event else {
                    return;
                };
