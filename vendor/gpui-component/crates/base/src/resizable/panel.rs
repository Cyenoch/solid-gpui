...
use super::{ResizableState, ResizeHandleRenderer, resize_handle};
...
pub struct ResizablePanelGroup {
    ...
    axis: Axis,
    size: Option<Pixels>,
    children: Vec<ComponentChild<ResizablePanel>>,
...
impl ResizablePanelGroup {
    ...
    pub fn child(mut self, panel: impl Into<ComponentChild<ResizablePanel>>) -> Self {
    ...
    where
        I: Into<ComponentChild<ResizablePanel>>,
    {
...
impl RenderOnce for ResizablePanelGroup {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        state.update(cx, |state, cx| {
            state.sync_panels(self.axis, &keys, cx);
            for (i, panel) in self.children.iter().enumerate() {
                state.configure_panel(i, panel.visible, panel.size_range.clone());
            }
        });
...
impl ResizablePanel {
    ...
    pub(super) fn new(key: ElementId) -> Self {
        Self {
            key,
            panel_ix: 0,
            initial_size: None,
    ...
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .on_prepaint({
                ...
                move |bounds, _, cx| {
                    state.update(cx, |state, cx| {
                        state.update_panel_size(&key, bounds, self.size_range, cx)
                    })
                }
            })
            .children(self.children)
            .when_some(
                state.read(cx).previous_visible(self.panel_ix),
                |this, ix| {
                    let before = state.read(cx).keys[ix].clone();
                    let after = self.key.clone();
                    this.child(
                        resize_handle((self.key.clone(), "resize-handle"), self.axis)
                            ...
                            .on_drag(DragPanel, move |drag_panel, _, _, cx| {
                                ...
                                state.update(cx, |state, _| {
                                    state.start_drag(before.clone(), after.clone());
                                });
    ...
    fn paint(
        ...
        window.on_mouse_event({
            ...
            move |e: &MouseMoveEvent, phase, window, cx| {
                ...
                state.update(cx, |state, cx| {
                    let Some(ix) = state.drag_index() else {
                        state.drag = None;
                        return;
                    };
        ...
        window.on_mouse_event({
            ...
            move |_: &MouseUpEvent, phase, window, cx| {
                if phase.bubble() && state.read(cx).drag_index().is_some() {
