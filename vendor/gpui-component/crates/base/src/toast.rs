...
impl<I: Clone + Eq, T> ToastManager<I, T> {
    /// Change a live toast's timeout without restarting its entrance or order.
    pub fn set_timeout(&mut self, id: &I, timeout: Option<Duration>) -> bool {
        let Some(entry) = self
            .entries
            .iter_mut()
            .find(|entry| &entry.id == id && entry.status != ToastTransitionStatus::Ending)
        else {
            return false;
        };
        entry.timeout_remaining = timeout;
        true
    }
