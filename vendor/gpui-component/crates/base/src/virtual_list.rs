...
impl VirtualListScrollHandle {
    ...
    /// The actual range prepared for the most recent frame, excluding measurement probes.
    pub fn visible_range(&self) -> Range<usize> {
        self.state.borrow().visible_range.clone()
    }
...
impl VirtualList {
    pub fn on_visible_range(
        ...
        self.on_visible_range = Some(Box::new(handler));
        self
    }
    ...
    fn scroll_to_deferred_item(
        ...
        let Some(ix) = scroll_to_item.item_index.checked_add(scroll_to_item.offset) else {
            return scroll_offset;
        };
        let Some((&origin, &extent)) = item_origins.get(ix).zip(item_sizes.get(ix)) else {
            return scroll_offset;
        };
        let bounds = Bounds {
            origin: match self.axis {
                Axis::Horizontal => point(content_bounds.left() + origin, content_bounds.top()),
                Axis::Vertical => point(content_bounds.left(), content_bounds.top() + origin),
            },
            size: match self.axis {
                Axis::Horizontal => size(extent, content_bounds.size.height),
                Axis::Vertical => size(content_bounds.size.width, extent),
            },
        };
...
impl Element for VirtualList {
            ...
            |style, window, cx| {
                size_layout = window.with_element_state(
                    ...
                    |state: Option<ItemSizeLayout>, _window| {
                        ...
                        if state.axis != Some(self.axis)
                            || state.gap != gap
                            || !Rc::ptr_eq(&state.items_sizes, &self.item_sizes)
                        {
    ...
    fn prepaint(
        ...
        if self.items_count == 0 {
            scroll_state.visible_range = 0..0;
        }
        ...
        self.base.interactivity().prepaint(
            ...
            |_style, _, hitbox, window, cx| {
                if self.items_count > 0 {
                    ...
                    let offset = scroll_offset.along(self.axis);
                    let leading_padding = if self.axis.is_vertical() {
                        paddings.top
                    } else {
                        paddings.left
                    };
                    let start = -(offset + leading_padding);
                    let end = -offset + content_bounds.size.along(self.axis);
                    // Cached prefix origins permit O(log n) range lookup on every scroll frame.
                    let first = item_origins
                        .partition_point(|origin| *origin < start)
                        .saturating_sub(1);
                    let last = item_origins
                        .partition_point(|origin| *origin < end)
                        .saturating_add(1)
                        .min(self.items_count);
                    let visible_range = first.min(self.items_count)..last;
                    self.scroll_handle.state.borrow_mut().visible_range = visible_range.clone();

                    if let Some(handler) = &self.on_visible_range {
...
mod tests {
    ...
    #[gpui::test]
    fn large_list_scrolling_builds_only_visible_rows(cx: &mut TestAppContext) {
        ...
        let (_, cx) = cx.add_window_view({
            ...
            move |_, _| VirtualListHarness {
                axis: Axis::Vertical,
                item_sizes: Rc::new(vec![size(px(80.), px(20.)); 100_000]),
        ...
        handle.scroll_to_item(99_990, ScrollStrategy::Top);
        cx.update(|window, cx| window.draw(cx).clear(cx));
        assert!(handle.visible_range().contains(&99_990));
        assert!(
            ranges.borrow().iter().map(|r| r.len()).sum::<usize>() < 16,
            "only the measurement probe and visible rows are built"
        );
        assert!(handle.offset().y < px(-1_900_000.));
    }
