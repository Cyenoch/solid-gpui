...
pub(crate) struct CommandModel {
    pub(crate) entries: Rc<Vec<CommandEntry>>,
...
impl CommandState {
    ...
    pub(crate) fn install_model(&mut self, model: CommandModel, cx: &mut Context<Self>) {
        if Rc::ptr_eq(&self.model.entries, &model.entries)
            && self.model.searchable == model.searchable
        ...
        let selected_key = self.selected_key().cloned();
        self.model = model;
                ...
                .find_map(|(matched_ix, matched)| {
                    (!matched.disabled
                        && match &selected_key {
                            Some(key) => self
                                .item_at(matched_ix)
                                .is_some_and(|item| item.key.as_ref() == Some(key)),
    ...
    pub fn selected_key(&self) -> Option<&SharedString> {
        self.selected_index
            .and_then(|i| self.item_at(i))
            .and_then(|i| i.key.as_ref())
    }
    pub fn project_selected_key(&mut self, key: Option<&str>, cx: &mut Context<Self>) {
        self.selected_index = key.and_then(|key| {
            self.matched.iter().enumerate().position(|(i, m)| {
                !m.disabled
                    && self
                        .item_at(i)
                        .is_some_and(|item| item.key.as_deref() == Some(key))
            })
        });
        self.preserve_no_selection = self.selected_index.is_none();
        self.pending_scroll = None;
        cx.notify();
    }
    fn resolve_item(&self, key: Option<&SharedString>, index: usize) -> Option<usize> {
        match key {
            Some(key) => self.matched.iter().enumerate().position(|(i, m)| {
                !m.disabled
                    && self
                        .item_at(i)
                        .is_some_and(|item| item.key.as_ref() == Some(key))
    ...
    fn render_item(
        ...
        let click_key = hover_key.clone();
        let id = item
            .key
            .clone()
            .map(gpui::ElementId::Name)
            ...
            .when(!disabled, |this| {
                this.cursor_default()
                    .on_hover(cx.listener(move |this, hovered: &bool, window, cx| {
                        if *hovered {
                            if let Some(index) = this.resolve_item(hover_key.as_ref(), matched_ix) {
                                this.select(index, window, cx);
                            }
                        }
                    }))
                    .on_click(cx.listener(move |this, _, window, cx| {
                        if let Some(index) = this.resolve_item(click_key.as_ref(), matched_ix) {
...
mod tests {
    ...
    fn action_that_removes_command_state_still_confirms_after_dispatch(cx: &mut TestAppContext) {
        ...
        cx.update(|window, cx| {
            ...
            let state = cx.new(|cx| {
                ...
                state.install_model(
                    CommandModel {
                        entries: Rc::new(vec![CommandEntry::Item(
                            CommandItem::new()
                                .label("removed")
    ...
    fn command_state(
        ...
        state.install_model(
            CommandModel {
                entries: Rc::new(entries.into_iter().collect()),
                ..CommandModel::default()
            },
            cx,
        );
    ...
    fn filterable_off_keeps_every_item_and_resets_the_highlight(cx: &mut TestAppContext) {
        ...
        cx.update(|window, cx| {
            let state = cx.new(|cx| {
                ...
                state.install_model(
                    CommandModel {
                        entries: Rc::new(suggestion_entries()),
                        filterable: false,
                        ..CommandModel::default()
                    },
                    cx,
