...
pub struct Command {
    ...
    selected_key: Option<Option<SharedString>>,
    searchable: bool,
    filterable: bool,
...
impl Command {
    ...
    pub fn entries(mut self, entries: Rc<Vec<CommandEntry>>) -> Self {
        self.entries = entries;
        self
    }
    /// Quietly apply an externally controlled highlight after installing the model.
    pub fn selected_key(mut self, key: Option<SharedString>) -> Self {
    ...
    pub fn item(mut self, item: CommandItem) -> Self {
        Rc::make_mut(&mut self.entries).push(CommandEntry::Item(item));
        self
    }

    /// Add multiple ungrouped command items.
    pub fn items(mut self, items: impl IntoIterator<Item = CommandItem>) -> Self {
        Rc::make_mut(&mut self.entries).extend(items.into_iter().map(CommandEntry::Item));
        self
    }

    /// Add a group of command items.
    pub fn group(mut self, group: CommandGroup) -> Self {
        Rc::make_mut(&mut self.entries).push(CommandEntry::Group(group));
        self
    }

    /// Add a separator between the preceding and following entries.
    pub fn separator(mut self) -> Self {
        Rc::make_mut(&mut self.entries).push(CommandEntry::Separator);
        self
    }
