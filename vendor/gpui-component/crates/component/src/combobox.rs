    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
            ...
            move |selected_item, _secondary, window, cx| {
                cx.defer_in(window, {
                    ...
                    move |list_state, window, cx| {
                        let Some((_, selected_item)) = selected_item else {
                            return;
                        };
                        let Some(index) = list_state
                            .delegate()
                            .delegate
                            .position(selected_item.value())
                        else {
                            return;
                        };
                        ...
                        for (index, selected) in &mut selection {
                            *index = list_state
                                .delegate()
                                .delegate
    ...
    pub fn set_multiple(&mut self, multiple: bool, cx: &mut Context<Self>) {
        self.multiple = multiple;
        if !multiple {
