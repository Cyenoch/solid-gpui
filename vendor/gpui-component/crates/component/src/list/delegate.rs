...
pub trait ListDelegate: Sized + 'static {
    ...
    /// Stable identity for resolving a rendered row after a model update.
    fn item_key(&self, ix: IndexPath, cx: &App) -> Option<gpui::SharedString> {
        None
    }
    fn index_for_key(&self, key: &str, cx: &App) -> Option<IndexPath> {
        None
    }
