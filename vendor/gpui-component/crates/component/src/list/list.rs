    ...
    D: ListDelegate,
{
    ...
    /// Disable automatic search selection for delegates that own selection by stable key.
    pub fn search_selects_first(mut self, enabled: bool) -> Self {
        self.search_selects_first = enabled;
        self
    }
    /// Project reconciled delegate state without emitting an interaction or moving scroll.
    pub fn project_selection(&mut self, ix: Option<IndexPath>, cx: &mut Context<Self>) {
    ...
    fn start_search(&mut self, query: String, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self._search_task = cx.spawn_in(window, async move |this, window| {
            ...
            _ = this.update_in(window, |this, window, cx| {
                if this.search_selects_first {
                    let first = (0..this.delegate.sections_count(cx)).find_map(|section| {
                        (0..this.delegate.items_count(section, cx))
                            .map(|row| IndexPath::default().section(section).row(row))
                            .find(|ix| this.delegate.can_select_index(*ix, cx))
                    });
    ...
    fn on_action_confirm(
        ...
        if !self.delegate.can_select_index(ix, cx) {
            return;
        }
        self.delegate
    ...
    fn select_item(&mut self, ix: IndexPath, window: &mut Window, cx: &mut Context<Self>) {
        if !self.selectable || !self.delegate.can_select_index(ix, cx) {
            return;
        }
    ...
    pub(crate) fn on_action_select_prev(
        ...
        let mut ix = self.selected_index;
        for _ in 0..self.rows_cache.items_count() {
            let next = self.rows_cache.prev(ix);
            if self.delegate.can_select_index(next, cx) {
                self.select_item(next, window, cx);
                break;
            }
            ix = Some(next);
        }
    ...
    pub(crate) fn on_action_select_next(
        ...
        let mut ix = self.selected_index;
        for _ in 0..self.rows_cache.items_count() {
            let next = self.rows_cache.next(ix);
            if self.delegate.can_select_index(next, cx) {
                self.select_item(next, window, cx);
                break;
            }
    ...
    fn render_list_item(
        ...
        let selectable = self.selectable && self.delegate.can_select_index(ix, cx);
        let row_key = self.delegate.item_key(ix, cx);
        ...
        div()
            ...
            .when(selectable, |this| {
                this.on_click(cx.listener(move |this, e: &ClickEvent, window, cx| {
                    let ix = match &click_key {
                        Some(key) => match this.delegate.index_for_key(key, cx) {
                            Some(ix) => ix,
                            None => return,
                        },
                        None => ix,
                    };
                    if !this.delegate.can_select_index(ix, cx) {
                        return;
                    }
