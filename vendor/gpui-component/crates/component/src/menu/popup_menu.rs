...
impl PopupMenu {
    ...
    /// Identity retained while builders run through another entity (for example a dock panel).
    pub fn owner(&self) -> WeakEntity<Self> {
        self.owner.clone()
    }
    ...
    /// Wire an existing submenu before publication, including when it moves
    /// between parents during keyed reconciliation.
    pub fn adopt_submenu(&self, submenu: &Entity<Self>, cx: &mut App) {
        let parent = self.owner.clone();
        submenu.update(cx, |menu, _| {
            menu.parent_menu = Some(parent);
            menu.priority = self.priority + 1;
        });
    }
    ...
    /// Reconcile the keyboard cursor after replacing items, without activating one.
    pub fn select_index(&mut self, index: Option<usize>, cx: &mut Context<Self>) {
    ...
    fn select_down(&mut self, _: &SelectDown, _: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(ix) = self.selected_index else {
            let first = self.clickable_menu_items().next().map(|(i, _)| i);
            self.select_index(first, cx);
            return;
        };
        ...
        for item in &self.menu_items {
            if let PopupMenuItem::Submenu { menu, .. } = item {
                if menu
                    ...
                    .parent_menu
                    .as_ref()
                    .is_none_or(|p| p.entity_id() != parent.entity_id())
                    || menu.read(cx).priority != parent_priority + 1
                {
