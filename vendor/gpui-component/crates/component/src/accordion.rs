...
use gpui::{
    ...
    SharedString, StyleRefinement, Styled, Window, div, percentage, prelude::FluentBuilder as _,
    rems,
};
...
pub struct Accordion {
    ...
    bordered: bool,
    disabled: bool,
    children: Vec<ComponentChild<AccordionItem>>,
...
impl Accordion {
    ...
    pub fn child(mut self, child: impl Into<ComponentChild<AccordionItem>>) -> Self {
...
impl RenderOnce for Accordion {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        let open_indices = Rc::new(RefCell::new(BTreeSet::new()));
            ...
            .children(
                self.children
                    ...
                    .map(|(ix, accordion)| {
                        ...
                        accordion.map_native(|accordion| {
                            let disabled = self.disabled || accordion.disabled;
                            let on_toggle = self.on_toggle_click.clone();
                            accordion
                                .index(ix)
                                .last(ix == last_ix)
                                .with_size(self.size)
                                .disabled(disabled)
                                .when_some(on_toggle, |accordion, on_toggle| {
                                    accordion.on_toggle_click({
                                        ...
                                        move |open, window, cx| {
                                            ...
                                            let next =
                                                open_indices.iter().copied().collect::<Vec<_>>();
                                            drop(open_indices);
