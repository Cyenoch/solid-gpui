    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
    ...
    pub fn new(
        ...
        let state = SearchableListState::new(
            ...
            move |selected_item, _secondary, window, cx| {
                cx.defer_in(window, {
                    let weak_confirm = weak_confirm.clone();
                    move |list_state, window, cx| {
                        let selected_index = selected_item.as_ref().and_then(|(_, item)| {
                            list_state.delegate().delegate.position(item.value())
                        });
                        if selected_item.is_some() && selected_index.is_none() {
                            return;
                        }
            ...
            move |_final_selected_index, window, cx| {
                cx.defer_in(window, {
                    ...
                    move |list_state, window, cx| {
                        let committed_ix = weak_cancel.upgrade().and_then(|e| {
                            e.read(cx)
                                .selected_value()
    ...
    /// Install an already-filtered model and value snapshots atomically.
    /// The caller resolves stable values against the full data and supplies the
    /// current filtered cursor. Query text, scroll handle and focus are retained.
    pub fn replace_model(
        ...
        self.state.list.update(cx, |list, cx| {
            ...
            list.delegate_mut().update_selection_snapshot(selection);
            list.set_selected_index(cursor, window, cx);
            cx.notify();
        });
        cx.notify();
    }
    pub fn cursor_value(&self, cx: &App) -> Option<<D::Item as SearchableListItem>::Value> {
        let list = self.state.list.read(cx);
        list.selected_index()
            .and_then(|ix| list.delegate().delegate.item(ix))
