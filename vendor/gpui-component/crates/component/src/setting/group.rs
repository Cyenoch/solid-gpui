...
pub struct SettingGroup {
    ...
    pub(super) items: Vec<ComponentChild<SettingItem>>,
}
...
impl SettingGroup {
    ...
    pub fn item(mut self, item: impl Into<ComponentChild<SettingItem>>) -> Self {
    ...
    where
        ...
        T: Into<ComponentChild<SettingItem>>,
    {
        ...
        GroupBox::new()
            ...
            .children(self.items.iter().enumerate().filter_map(|(item_ix, item)| {
                if item.is_match(&query, cx) {
                    Some(item.clone().render_deferred({
                        let options = options.with_item_ix(item_ix);
                        move |item, window, cx| item.render_item(&options, window, cx)
                    }))
                } else {
                    None
                }
