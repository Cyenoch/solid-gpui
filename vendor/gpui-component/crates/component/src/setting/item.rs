use gpui::{
    AnyElement, App, Axis, Div, ElementId, InteractiveElement as _, IntoElement, ParentElement,
    RenderOnce, SharedString, Stateful, Styled, Window, div, prelude::FluentBuilder as _,
};
...
impl SettingItem {
    ...
    pub fn new<F>(key: impl Into<ElementId>, title: impl Into<SharedString>, field: F) -> Self
    where
    ...
    pub fn render<R, E>(key: impl Into<ElementId>, render: R) -> Self
    where
        E: IntoElement,
    ...
    pub(super) fn render_item(
        ...
        let key = match &self {
            Self::Item { key, .. } | Self::Element { key, .. } => key.clone(),
        };
        div().id(key).w_full().child(match self {
            SettingItem::Item {
                title,
                description,
                layout,
                ...
                div()
                    ...
                    .child(div().id("field").child(DeferredField {
                        field,
                        options: options.with_layout(layout).with_disabled(disabled),
                    }))
                    .into_any_element()
            }
            SettingItem::Element {
...
impl RenderOnce for DeferredField {
    fn render(self, _: &mut Window, _: &mut App) -> impl IntoElement {
        self.field.render_boundary(
            FieldContents {
...
impl RenderOnce for FieldContents {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        SettingItem::render_field(self.field, self.options, window, cx)
    }
}
