...
pub struct Breadcrumb {
    ...
    items: Vec<ComponentChild<BreadcrumbItem>>,
}
...
impl Breadcrumb {
    ...
    pub fn child(mut self, item: impl Into<ComponentChild<BreadcrumbItem>>) -> Self {
    ...
    pub fn children(
        mut self,
        items: impl IntoIterator<Item = impl Into<ComponentChild<BreadcrumbItem>>>,
    ) -> Self {
...
impl RenderOnce for Breadcrumb {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        for (ix, item) in self.items.into_iter().enumerate() {
            ...
            children.push(
                item.map_native(|item| item.id(ix).is_last(is_last))
                    .into_any_element(),
            );
            if !is_last {
