use crate::{ComponentChild, StyledExt};
use gpui::{
...
pub struct DescriptionList {
    items: Vec<ComponentChild<DescriptionItem>>,
    size: Size,
    layout: Axis,
...
impl Styled for DescriptionItem {
...
impl DescriptionItem {
    ...
    pub fn separator() -> Self {
        Self {
            kind: DescriptionKind::Separator,
            style: Default::default(),
        }
    }
    ...
    pub fn value(mut self, value: impl Into<DescriptionText>) -> Self {
        ...
        if let DescriptionKind::Item { value, .. } = &mut self.kind {
            *value = new_value;
        }
        self
    }
    ...
    ///
    /// This method only works for [`DescriptionKind::Item`].
    pub fn span(mut self, span: usize) -> Self {
        let val = span;
        if let DescriptionKind::Item { span, .. } = &mut self.kind {
            *span = val;
        }
        self
    }
    ...
    fn _label(&self) -> Option<&DescriptionText> {
        match &self.kind {
            DescriptionKind::Item { label, .. } => Some(label),
            _ => None,
        }
    }
    ...
    fn _span(&self) -> Option<usize> {
        match &self.kind {
            DescriptionKind::Item { span, .. } => Some(*span),
            _ => None,
        }
    }
}
...
impl DescriptionList {
    ...
    /// Add a [`DescriptionKind::Item`] to the list.
    pub fn item(
        mut self,
        ...
        self.items
            .push(DescriptionItem::new(label).value(value).span(span).into());
        self
    }

    /// Add a child to the list.
    pub fn child(mut self, child: impl Into<ComponentChild<DescriptionItem>>) -> Self {
    ...
    pub fn children(
        mut self,
        children: impl IntoIterator<Item = impl Into<ComponentChild<DescriptionItem>>>,
    ) -> Self {
        self.items
    ...
    pub fn separator(mut self) -> Self {
        self.items.push(DescriptionItem::separator().into());
        self
    }
    ...
    fn group_item_rows(
        items: Vec<ComponentChild<DescriptionItem>>,
        columns: usize,
    ) -> Vec<Vec<ComponentChild<DescriptionItem>>> {
        let mut rows = vec![];
                ...
                h_flex()
                    ...
                    .children({
                        items.into_iter().enumerate().map(|(item_ix, item)| {
                            ...
                            item.render_with(|item| {
                                let style = item.style;
                                let element = match item.kind {
                                    DescriptionKind::Item { label, value, span } => {
                                        ...
                                        el.flex_1()
                                            .flex_basis(relative(
                                                (span as f32) / (self.columns as f32),
                                            ))
                                            .overflow_x_hidden()
                                            .child(
                                                div()
...
mod tests {
    ...
    fn test_group_item_rows() {
        ...
        let rows =
            super::DescriptionList::group_item_rows(items.into_iter().map(Into::into).collect(), 3);
