    ...
    T: PartialEq,
{
    ...
    /// The coordinate of a known domain position, without a linear value lookup.
    pub fn tick_at(&self, index: usize) -> Option<f32> {
        if index >= self.domain.len() {
            return None;
        }
        Some(if self.domain.len() == 1 {
            self.range_start + self.range_tick / 2.
        } else {
            self.range_start + index as f32 * self.range_tick
        })
    }
}
