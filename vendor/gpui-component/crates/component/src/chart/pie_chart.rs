...
impl<T> PieChart<T> {
    ...
    fn get_outer_radius(&self, arc: &ArcData<T>, automatic: f32) -> f32 {
        let radius = self
            .outer_radius_fn
            .as_ref()
            .map_or(self.outer_radius, |f| f(arc));
        if radius.is_zero() { automatic } else { radius }
    }
...
impl<T> Plot for PieChart<T> {
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        let label_radius = arcs
            .iter()
            .map(|arc| self.get_outer_radius(arc, outer_radius))
            .fold(outer_radius, f32::max)
            + self.label_gap;
