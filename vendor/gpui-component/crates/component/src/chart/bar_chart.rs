    ...
    V: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    ...
    fill: Option<Rc<dyn Fn(&T, Bounds<f32>, Bounds<f32>, BarAlignment) -> Option<Background>>>,
    ...
    V: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
        ...
        Bg: Into<Background> + 'static,
    {
        self.fill = Some(Rc::new(move |t, bar_bounds, chart_bounds, alignment| {
            Some(fill(t, bar_bounds, chart_bounds, alignment).into())
        }));
        self.fill_gradient = None;
        self
    }

    /// Override selected bars while unspecified values use the current theme color.
    pub fn fill_optional(
        mut self,
        fill: impl Fn(&T, Bounds<f32>, Bounds<f32>, BarAlignment) -> Option<Background> + 'static,
    ) -> Self {
        self.fill = Some(Rc::new(fill));
        self.fill_gradient = None;
        self
    }
    ...
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        bar = match (fill, fill_gradient) {
            ...
            (Some(f), _) => bar.fill(move |d, frame, alignment| {
                f(d, frame, chart_bounds, alignment).unwrap_or(default_fill)
            }),
