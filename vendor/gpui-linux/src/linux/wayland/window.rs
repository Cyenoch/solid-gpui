...
pub struct WaylandWindowState {
    ...
    /// Children in creation order, with modal input policy. Destroy topmost first.
    children: Vec<(ObjectId, bool)>,
    destroyed: bool,
    shown: bool,
    mapped: bool,
...
impl WaylandSurfaceState {
    fn new(
        ...
        if let WindowKind::AnchoredPopup(options) = &params.kind {
            anyhow::ensure!(
                globals.wm_base.version() >= 3,
                "anchored popups require xdg-shell version 3 for repositioning"
            );
...
impl Drop for WaylandWindow {
    fn drop(&mut self) {
        // Retain the native state until close can retire descendant roles and
        // deliver their close callbacks before destroying this owner's role.
...
impl WaylandWindowStatePtr {
    ...
    fn complete_frame(&self) {
        let mut state = self.state.borrow_mut();
        if state.destroyed || !state.shown {
            self.frame_loop.set(FrameLoop::Parked);
            return;
        }
    ...
    fn children_topmost_first(&self) -> Vec<WaylandWindowStatePtr> {
        let state = self.state.borrow();
        ...
        state
            ...
            .iter()
            .rev()
            .filter_map(|(id, _)| get_window(&mut client.borrow_mut(), id))
            .collect()
    }

    fn destroy_native(&self) {
        {
            let mut state = self.state.borrow_mut();
            if state.destroyed {
                return;
        ...
        self.frame_loop.set(FrameLoop::Parked);
        // Protocol role destructors must run before the owner's destructor, even
        // when GPUI's window-close callback defers removal to a later turn.
        for child in self.children_topmost_first() {
    ...
    pub fn close(&self) {
        let children = self.children_topmost_first();
...
impl PlatformWindow for WaylandWindow {
    ...
    fn reposition_popup(&mut self, anchor: Bounds<Pixels>) -> anyhow::Result<()> {
        ...
        anyhow::ensure!(!state.destroyed, "popup was destroyed");
        let mapped = state.mapped;
        let WaylandSurfaceState::Popup(popup) = &mut state.surface_state else {
            anyhow::bail!("window is not an anchored popup");
        };
        popup.options.anchor_rect = anchor;
        popup.pending_reposition = !mapped;
        let size = popup.requested_size;
        if mapped {
            let parent = state
                .parent
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("popup parent is unavailable"))?;
            state
                .surface_state
                .reposition_popup(&state.globals, size, parent.window_geometry());
        }
        Ok(())
    }
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        {
            let mut state = self.borrow_mut();
            if let WaylandSurfaceState::Popup(popup) = &mut state.surface_state {
    ...
    fn activate(&self) {
        ...
        // xdg-activation applies to toplevels. An ungrabbed popup's keyboard
        // focus is compositor policy; never fabricate an input-grab serial.
        if matches!(self.borrow().surface_state, WaylandSurfaceState::Popup(_)) {
            return;
        }
    ...
    fn draw(&self, scene: &Scene) {
        ...
        if state.renderer.draw(scene) {
            state.mapped = true;
            let pending_size = if let WaylandSurfaceState::Popup(popup) = &mut state.surface_state {
                std::mem::take(&mut popup.pending_reposition).then_some(popup.requested_size)
            } else {
                None
            };
