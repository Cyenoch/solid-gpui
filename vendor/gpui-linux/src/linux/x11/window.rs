...
use gpui::{
    ...
    WindowDecorations, WindowKind, WindowParams, point,
    popup::{self, PopupOptions},
    px, size,
};
...
pub struct X11WindowState {
    ...
    children: Vec<(xproto::Window, bool)>,
    popup: Option<(PopupOptions, Size<Pixels>)>,
    shown: bool,
...
impl X11WindowState {
    pub fn new(
        ...
        let popup = if let WindowKind::AnchoredPopup(ref options) = params.kind {
            anyhow::ensure!(!options.grab, "explicit popup grabs are unavailable on X11");
            anyhow::ensure!(
                parent_window.is_some(),
                "popup parent window is unavailable"
            );
            Some((options.clone(), params.bounds.size))
        } else {
            None
        };
        ...
        let win_aux = xproto::CreateWindowAux::new()
            ...
            .colormap(colormap)
            .override_redirect((params.kind == WindowKind::PopUp || popup.is_some()) as u32)
            .event_mask(
        ...
        let setup_result = maybe!({
            ...
            if params.kind == WindowKind::PopUp || popup.is_some() {
                check_reply(
            ...
            let parent = if (params.kind == WindowKind::Dialog || popup.is_some())
                && let Some(parent) = parent_window
            {
                parent.add_child(x_window, params.kind == WindowKind::Dialog);
    ...
    fn drop(&mut self) {
        ...
        if let Some(parent) = state.parent.as_ref() {
            parent
                ...
                .borrow_mut()
                .children
                .retain(|(id, _)| *id != self.0.x_window);
        }
...
impl X11Window {
    pub fn new(
        ...
        let anchor = ptr
            ...
            .popup
            .as_ref()
            .map(|(options, _)| options.anchor_rect);
        let mut window = Self(ptr);
        if let Some(anchor) = anchor {
            window.reposition_popup(anchor)?;
        }
        Ok(window)
    }
...
impl X11WindowStatePtr {
    ...
    pub fn add_child(&self, child: xproto::Window, blocking: bool) {
        let mut state = self.state.borrow_mut();
        state.children.push((child, blocking));
    }

    pub fn is_blocked(&self) -> bool {
        let state = self.state.borrow();
        state.children.iter().any(|(_, blocking)| *blocking)
    }
...
impl PlatformWindow for X11Window {
    ...
    fn reposition_popup(&mut self, anchor: Bounds<Pixels>) -> anyhow::Result<()> {
        let (mut options, requested, parent, root, scale, atoms) = {
            let state = self.0.state.borrow();
            let (options, requested) = state
                .popup
                .as_ref()
                .context("window is not an anchored popup")?;
            (
                options.clone(),
                *requested,
                state.parent.clone().context("popup owner is unavailable")?,
                state.x_root_window,
                state.scale_factor,
                state.atoms,
            )
        ...
        let origin = self
            .0
            .xcb
            .translate_coordinates(parent.x_window, root, 0, 0)?
            .reply()?;
        let mut placement = options.clone();
        placement.anchor_rect = Bounds::new(
            point(px(origin.dst_x as f32), px(origin.dst_y as f32)) + anchor.origin * scale,
            anchor.size.map(|value| value * scale),
        );
        placement.offset = placement.offset * scale;
        let work = popup_work_area(&self.0.xcb, root, &atoms, placement.anchor_rect)?;
        let bounds = popup::popup_bounds(&placement, requested.map(|value| value * scale), work);
        self.0
            .xcb
            .configure_window(
                self.0.x_window,
                &xproto::ConfigureWindowAux::new()
                    ...
                    .width(bounds.size.width.as_f32().ceil() as u32)
                    .height(bounds.size.height.as_f32().ceil() as u32),
            )?
            .check()?;
        self.0.state.borrow_mut().popup = Some((options, requested));
        self.0.xcb.flush()?;
        Ok(())
    }
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        let anchor = self
            ...
            .popup
            .as_mut()
            .map(|(options, requested)| {
                *requested = size;
                options.anchor_rect
            });
        if let Some(anchor) = anchor {
            self.reposition_popup(anchor).log_err();
            return;
        }
        let state = self.0.state.borrow();
    ...
    fn activate(&self) {
        if self.0.state.borrow().popup.is_some() {
            self.0.state.borrow_mut().shown = true;
            let xcb = &self.0.xcb;
            let result = (|| -> anyhow::Result<()> {
                xcb.map_window(self.0.x_window)?.check()?;
...
/// X11 uses one desktop scale. Placement stays in root-device coordinates and
/// constrains to the active RandR monitor intersected with the WM's work area.
fn popup_work_area(
    xcb: &XCBConnection,
    root: xproto::Window,
    atoms: &XcbAtoms,
    ...
    let resources = xcb.randr_get_screen_resources_current(root)?.reply()?;
    let monitors = resources
        .crtcs
        .iter()
        .filter_map(|crtc| {
            let info = xcb
                ...
                .reply()
                .ok()?;
            (info.width > 0 && info.height > 0).then(|| {
                Bounds::new(
                    point(px(info.x as f32), px(info.y as f32)),
                    size(px(info.width as f32), px(info.height as f32)),
                )
            })
        })
        .collect::<Vec<_>>();
    let monitor = monitors[popup::popup_display(anchor, &monitors)
        .context("popup requires an active RandR monitor")?];
    let desktop = xcb
        .get_property(
            false,
            root,
    ...
    let work = xcb
        .get_property(
            ...
            desktop
                .checked_mul(4)
                .context("invalid desktop work area index")?,
            4,
        )?
        .reply()?;
    if let Some(values) = work.value32() {
        let values: Vec<_> = values.collect();
        if let [x, y, width, height] = values.as_slice() {
            let area = Bounds::new(
                point(px(*x as i32 as f32), px(*y as i32 as f32)),
                size(px(*width as f32), px(*height as f32)),
            );
            let intersection = monitor.intersect(&area);
            if intersection.size.width > px(0.0) && intersection.size.height > px(0.0) {
                return Ok(intersection);
            }
        }
    }
    // WMs without EWMH work-area hints expose only the monitor bounds.
    Ok(monitor)
}
