...
impl WindowsPlatform {
    ...
    fn find_current_active_window(&self) -> Option<HWND> {
        let active_window_hwnd = unsafe { GetForegroundWindow() };
...
impl Platform for WindowsPlatform {
    ...
    fn open_window(
        ...
        let owner = if let gpui::WindowKind::AnchoredPopup(ref popup) = options.kind {
            Some(
                self.raw_window_handles
                    .read()
                    .iter()
                    ...
                    .ok_or_else(|| anyhow::anyhow!("popup owner window is unavailable"))?,
            )
        } else {
            None
        };
        let window = WindowsWindow::new(handle, options, self.generate_creation_info(), owner)?;
