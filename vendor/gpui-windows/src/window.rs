...
pub(crate) struct WindowsWindowInner {
    ...
    popup: RefCell<Option<WindowsPopup>>,
}
...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        owner: Option<Rc<WindowsWindowInner>>,
    ) -> Result<Self> {
        let popup = if let WindowKind::AnchoredPopup(ref options) = params.kind {
            anyhow::ensure!(
                !options.grab,
                "explicit popup grabs are unavailable on Windows"
            );
            Some(WindowsPopup {
                options: options.clone(),
                requested_size: params.bounds.size,
                owner: Rc::downgrade(owner.as_ref().context("popup owner is unavailable")?),
            })
        } else {
            None
        };
        ...
        let (mut dwexstyle, dwstyle) = if popup.is_some() {
        ...
        let creation_result = unsafe {
            CreateWindowExW(
                ...
                CW_USEDEFAULT,
                CW_USEDEFAULT,
                owner.as_ref().map(|owner| owner.hwnd).or(parent_hwnd),
                None,
                Some(hinstance.into()),
        ...
        if let Some(popup) = popup {
            let anchor = popup.options.anchor_rect;
            *this.popup.borrow_mut() = Some(popup);
            let mut window = Self(this);
...
impl PlatformWindow for WindowsWindow {
    ...
    fn reposition_popup(&mut self, anchor: Bounds<Pixels>) -> Result<()> {
        let mut popup = self
            .popup
            .borrow()
            .clone()
        ...
        let owner = popup
            .owner
            .upgrade()
            .context("popup owner is unavailable")?;
        anyhow::ensure!(
            unsafe { IsWindow(Some(owner.hwnd)) }.as_bool(),
            "popup owner was destroyed"
        );
        popup.options.anchor_rect = anchor;
        let owner_scale = owner.state.scale_factor.get();
        let mut origin = POINT {
            x: (anchor.origin.x.as_f32() * owner_scale).round() as i32,
            y: (anchor.origin.y.as_f32() * owner_scale).round() as i32,
        };
        unsafe {
            ClientToScreen(owner.hwnd, &mut origin).ok()?;
        }
        let rect = RECT {
            left: origin.x,
            top: origin.y,
            right: origin.x + (anchor.size.width.as_f32() * owner_scale).ceil() as i32,
            bottom: origin.y + (anchor.size.height.as_f32() * owner_scale).ceil() as i32,
        };
        let monitor = unsafe { MonitorFromRect(&rect, MONITOR_DEFAULTTONEAREST) };
        let display = WindowsDisplay::new(WindowsDisplay::display_id_for_monitor(monitor))
            .context("popup display is unavailable")?;
        ...
        let mut placement = popup.options.clone();
        placement.anchor_rect = Bounds::new(
            point(px(origin.x as f32), px(origin.y as f32)),
            anchor.size.map(|value| value * owner_scale),
        );
        placement.offset = placement.offset * scale;
        let bounds = popup::popup_bounds(
            &placement,
            popup.requested_size.map(|value| value * scale),
            Bounds::new(
                point(px(work.left as f32), px(work.top as f32)),
                size(
                    px((work.right - work.left) as f32),
                    px((work.bottom - work.top) as f32),
                ),
            ),
        );
        // Release the borrow before SetWindowPos synchronously dispatches DPI/size messages.
        *self.popup.borrow_mut() = Some(popup);
        unsafe {
            SetWindowPos(
                self.hwnd,
                None,
                ...
                bounds.size.width.as_f32().ceil() as i32,
                bounds.size.height.as_f32().ceil() as i32,
    ...
    fn resize(&mut self, size: Size<Pixels>) {
        let anchor = self.popup.borrow_mut().as_mut().map(|popup| {
            popup.requested_size = size;
            popup.options.anchor_rect
        });
