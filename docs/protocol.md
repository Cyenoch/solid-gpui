...
|    6 | CommandResult     | `[2,requestId,command,nodeId,success,error,value|null]`                              | Tag `2`; command must be `1..24`; success bool; error string/null; value is null when the command has no typed return value, otherwise it uses the tags below. | `protocol.ts:209-217,529-566`; `wire/event.rs:318-335,370-393,617-705` |
...
|   4 | `[4,string]`         | UTF-8 string up to 1 MiB. Used by clipboard-read.                                                                                                                                        | Same sources; `commands.rs`                         |
