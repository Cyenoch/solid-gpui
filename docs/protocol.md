...
|   23 | SetClosePolicy      | Root-only (`nodeId=1`)                                               | string policy name, currently `require-confirmation`                                                                              | Configures whether native close requests emit Event 23 for JavaScript resolution; unsupported policy names are rejected.                                                                                                                                | `root-container.ts`; `wire/command.rs`; `host/main.rs`                                  |
