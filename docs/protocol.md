...
| 6 | CommandResult | `[2,requestId,command,nodeId,success,error,value|null]` | Tag `2`; command must be one of the implemented command codes `1..33`; success is boolean, error is string or null, and value is null or one of the typed result tags below. | `protocol.ts`; `wire/event.rs` |
...
The command directory below defines all 33 command kinds (`1..33`).
