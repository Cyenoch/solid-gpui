...
|        8 | u32                    | Event type `1..23`; the payload at position 9 is validated according to this value. `EVENT_SURFACE_CLOSED` additionally requires node/listener `0/0`; `EVENT_CLOSE_REQUESTED` requires node `1`/listener `0`; generic View/Pressable/Text Focus/Blur uses a null payload and positive node/listener IDs. | `protocol.ts:687-693`; `wire/event.rs:20-171`             |
...
|        9 | `focusable`      | boolean               | Current retained-tree validation permits focusability on View, Pressable, and listener-bearing Text nodes; focused Text requires a nonzero `listenerId`.                                                                                                  | `tree/validation.rs:105-115`                                                     |
...
nested Text listener. Listener-bearing nested Text runs in non-selectable rich
text receive native focus handles and tab stops and emit the existing focus/blur
events; Enter synthesizes their existing Press event, while Space is not
synthesized. A visible per-run focus affordance remains a renderer boundary.
...
## 3. Event directory
The event directory below defines all 23 event kinds (`1..23`); pointer down/up/move are payload variants within event kind `10`.
...
|                                                                                 6 | CommandResult | `[2,requestId,command,nodeId,success,error,value|null]` | Tag `2`; command must be one of the implemented command codes `1..35`; success is boolean, error is string or null, and value is null or one of the typed result tags below. | `protocol.ts`; `wire/event.rs` |
...
The command directory below defines all 35 command kinds (`1..35`).
...
|   23 | SetClosePolicy      | Root-only (`nodeId=1`)                                               | string policy name: `allow` or `require-confirmation`                                                                              | Configures whether native close requests are allowed immediately or emit Event 23 for JavaScript resolution; unsupported policy names are rejected.                                                                                                      | `root-container.ts`; `wire/command.rs`; `host/main.rs`                                  |
...
|   27 | ClipboardWriteImage | Root-only (`nodeId=1`)                                               | `[formatCode,bin]`, with format `1=png`, `2=jpeg`, `3=gif`, or `4=svg`; non-empty binary bytes capped at `MAX_CLIPBOARD_IMAGE_BYTES = MAX_FRAME_SIZE - 1024`                                                        | Writes an encoded image through GPUI's native clipboard image entry. macOS and Windows support this path; X11 and Wayland return `clipboard image command is unsupported on this platform; use text clipboard commands or run on macOS/Windows`.                                                                                                                                                                                         | `root-container.ts`; `wire/command.rs`; `renderer/commands.rs`                               |
|   28 | ClipboardReadImage  | Root-only (`nodeId=1`)                                               | `null`                                                                                                                                                                                                              | Returns value tag `7` with the original encoded format and bytes; empty/non-image clipboard contents resolve as `null` in TypeScript. macOS and Windows reject this command with `clipboard image command is unsupported on this platform; use text clipboard commands or run on macOS/Windows`.                                                                                                                                                 | `root-container.ts`; `wire/event.rs`; `renderer/commands.rs`                                 |
...
|   34 | GetScrollOffset     | Node-level; VirtualList                                              | `null`                                                                                                                                                                                                              | Reads the native list pixel displacement and returns the effective logical offset; see the VirtualList scroll command semantics above.                                                                                                                                                                                                                              | `docs/protocol.md:231-248`; `root-container.ts`; `wire/command.rs`; `wire/event.rs`; `renderer/commands.rs` |
|   35 | ScrollToOffset      | Node-level; VirtualList                                              | finite, non-negative logical-pixel number                                                                                                                                                                             | Writes the native scrollbar offset with native clamping preserved; see the VirtualList scroll command semantics above.                                                                                                                                                                                                                                             | `docs/protocol.md:231-248`; `root-container.ts`; `wire/command.rs`; `renderer/commands.rs`             |
