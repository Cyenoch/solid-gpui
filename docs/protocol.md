...
| ------------------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Snapshot `[6]`           | array of Node tuples                                                                                           | Complete node set. Each node has ten required fields and may carry optional `selectable` and `tooltip` tails; shape and parent/child relationship are validated by the retained tree.                                                             | `protocol.rs:81`; `tree.rs:215-303; tree/validation.rs:81-229,231-273`                               |
...
| Create operation `[0]=1` | `[1,id,parentId,index,kind,style,text,listenerId,hostProperties,accessibility,focusable,selectable?,tooltip?]` | Same node fields as Snapshot. The optional tail is omitted when both values are false/absent; a tooltip always includes the selectable placeholder, even when false.                                                                              | `protocol.ts:123-136`; `wire/snapshot_patch.rs:90-116,136-153,182-216`                               |
| Update operation `[0]=2` | `[2,id,mask,style,text,listenerId,hostProperties,accessibility,focusable,selectable?,tooltip?]`                | `mask` selects changed fields: style `1`, text `2`, listener `4`, host properties `8`, accessibility `16`, focusable `32`, selectable `64`, tooltip `128`; a tooltip update always appends `[selectable,tooltip]`, with `null` clearing the text. | `protocol.ts:69-74,137-147`; `protocol.rs:60-65`; `wire/snapshot_patch.rs:90-130,155-176,218-239`    |
...
|                                                                                 6 | CommandResult        | `[2,requestId,command,nodeId,success,error,value            | null]`                                                                                                                                                                                                                                                                                                                                                      | Tag `2`; command must be one of the implemented command codes `1..28`; success bool; error string/null; value is null when the command has no typed return value, otherwise it uses the tags below.                        | `protocol.ts:209-217,529-566`; `wire/event.rs:318-335,617-705`                                                             |
...
|                                                                                10 | Pointer              | `[6,button,modifiers,action,clickCount,x,y]`                | Tag `6`; button `1=left` through `5=forward`; unique known modifiers; action `1=down`/`2=up`; clickCount > 0 u32; `x`/`y` are required finite non-negative logical window pixels. The host clamps native coordinates to the viewport before encoding. Coordinates are emitted for press/release only; hover/move coordinates are intentionally not exposed. | `protocol.ts:323,723-754`; `wire/event.rs:468,550-600`; `renderer/paint/mod.rs:286-320`                                                                                                                                    |
...
|   4 | `[4,string]`           | UTF-8 string up to 1 MiB. Used by ClipboardRead.                                                                                                                                         | Same sources; `commands.rs`                   |
...
|   23 | SetClosePolicy      | Root-only (`nodeId=1`)                                               | `"allow"` or `"require-confirmation"`                                                                                                                                                                               | Sets the per-surface native close policy. `allow` permits the synchronous native close; `require-confirmation` vetoes it and emits Event 23 for an asynchronous JavaScript decision. Changing policy clears any pending request.                                                                                                                                          | `root-container.ts`; `wire/command.rs:112-125,307-312`; `host/main.rs`                       |
