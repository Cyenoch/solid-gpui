...
核心 VirtualList 对项身份而非仅数量计版本。追加、前插和替换区间以高度估值增量拼接原生状态，保留区间外测量结果，并随保留内容移动逻辑锚点。响应式 store 和显式重新发布的可变数组使用独立身份快照；仅视口变化不再次复制全部数据。没有响应式身份变化的对象内部原地修改不算数据替换，但已渲染行的响应式内容仍触发原生重新测量。完整 Snapshot 和估值变化是显式失效边界。成本及 CPU 对照见[批量工作与保留几何](performance-analysis.zh-CN.md#批量工作与保留几何)。
