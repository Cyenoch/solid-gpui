...
(`crates/solid-gpui/src/tree/validation.rs:274-296,369-404`) and are part of
...
`lineHeight` (`crates/solid-gpui/src/tree/validation.rs:298-368`). The pinned
...
(`crates/solid-gpui/src/renderer/paint/text_input.rs:29-65,893-918`; listener
allocation is in `packages/solid-gpui/src/renderer/nodes.ts:282-307`). The
...
(`crates/solid-gpui/src/renderer/paint/text_input.rs:938-959`,
`crates/solid-gpui/src/renderer/paint/mod.rs:229-250`). This is required by the
...
(`crates/solid-gpui/src/renderer/paint/text_input.rs:960-976`). `Space` is not
...
`apply_style_without_cursor`, `crates/solid-gpui/src/renderer/paint/text_input.rs:864-882`).
...
bytes (`crates/solid-gpui/src/renderer/paint/text_input.rs:983-1014`,
`crates/solid-gpui/src/renderer/input.rs:323-427`). This extends the selectable
...
(`crates/solid-gpui/src/renderer/paint/text_input.rs:187-219`). They are focus
...
(`crates/solid-gpui/src/renderer.rs:248-360`). This is the targeted invalidation
...
We do not add a SolidRoot-level shaped-layout cache. The split benchmark found
...
- Range geometry must use the shared shaped layout and global UTF-8 offsets;
  ...
  (`crates/solid-gpui/src/renderer/input.rs:323-427`).
...
- Selection, link activation, cursor choice, and focus affordances depend on
  ...
  boundary.
- The host owns transient focus affordance and selectable-range state; SolidJS
