...
| `Pressable`      | `children`, `style`, `onPress`, `focusable`, keyboard/pointer/hover/focus/layout handlers, `disabled`, `draggable`, drag/drop handlers, `tooltip`, accessibility props                               | Pointer and native keyboard activation share the press path. Focus callbacks require `focusable`; `disabled` removes interaction. The separate `accessibilityDisabled` field is retained on the wire, but pinned GPUI does not currently expose a public AccessKit disabled-state builder.                                                                    |
