...
const appName = "Solid GPUI";
const identifier = "io.github.cyenoch.solid-gpui-website";
...
async function main(): Promise<void> {
  ...
  if (values.help) {
    console.log("Usage: bun scripts/website-package.ts [--out <directory>]");
    return;
  }
  ...
  const version: string = metadata.packages.find((item: { name: string }) => item.name === "website-host").version;
  if (!/^\d+\.\d+\.\d+$/.test(version)) throw new Error("Website packages require a MAJOR.MINOR.PATCH version");

  const output = resolve(values.out ?? join(root, "dist/website"));
...
<plist version="1.0"><dict>
  ...
  <key>CFBundleIconFile</key><string>solid-gpui.icns</string>
...
`,
      ...
      await copy(join(root, "assets/branding/solid-gpui.icns"), join(stage, resources, "solid-gpui.icns"));
...
`,
      ...
      for (const size of [128, 256, 512]) {
        await copy(
          join(root, `assets/branding/icon-${size}.png`),
          join(stage, `usr/share/icons/hicolor/${size}x${size}/apps/${identifier}.png`),
        );
      }
    } else {
    ...
    console.log(`Website package verified: ${destination}`);
  } finally {
