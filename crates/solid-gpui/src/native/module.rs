...
impl CommandDefinition {
    ...
    where
        ...
        F: Fn(I, NativeCallContext) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = Result<O, String>> + Send + 'static,
    {
        ...
        Self {
            ...
            handler: CommandHandler::Worker(Arc::new(move |bytes, executor| {
                ...
                executor.asynchronous(move |context| async move {
                    let request = decode_json(&bytes)?;
                    encode_json(&function(request, context).await?)
                })
            })),
        }
    }
...
const TYPESCRIPT_HEADER: &str = "// Generated by the running Rust host. Do not edit.\nimport { createNativeComponent, createNativeClient, useNativeClient, type NativeCallOptions } from '@solid-gpui/core/native';\n";
    ...
    value
        ...
        .map(|c| {
            format!(
                ...
                if c["input"] == "null" {
                    "request?: undefined, options?: NativeCallOptions".into()
                } else {
                    format!(
                        "request: {}, options?: NativeCallOptions",
