...
fn command(
    ...
    let operation = match CommandKind::try_from(kind).expect("known property command kind") {
        ...
        kind => panic!("unsupported property command {kind:?}"),
    };
    Command::new(
        CommandMeta {
            surface_id,
...
fn command_result(runtime: &InMemoryAdapter, request_id: u32) -> CommandResult {
    loop {
        ...
        if event.payload.event_kind() == crate::protocol::EventKind::CommandResult
            && let EventPayload::CommandResult(result) = event.payload
...
fn send_command(
    ...
    let request_id = command.meta.request_id;
