use std::sync::Arc;
use gpui::{AnyElement, App, Element, Entity, ImageCacheError, ImageRequest, ImageSource,
    InteractiveElement, ManagedImageFrame, ManagedImageSource, ObjectFit, Pixels, Size,
...
#[derive(Default)]
struct ImageState {
    identity: Option<(ImageKey, Vec<(ImageKey, (u32,u32))>)>,
...
fn target_size(size: (u32,u32), request: ImageRequest) -> (u32,u32) {
    ...
    (bucket(size.0 as f32*x*request.scale_factor),bucket(size.1 as f32*y*request.scale_factor))
}

impl ImageProvider {
    fn with_state<R>(&self, window:&mut Window, cx:&mut App, f:impl FnOnce(&mut ImageState,&mut Window,&mut App)->R)->R {
        window.with_global_id("solid-gpui-image-resource".into(),|id,window| {
            window.with_element_state(id,|state:Option<ImageState>,window| {
                ...
                if state.identity.as_ref().is_none_or(|(source,variants)|source!=&self.source || variants!=&self.variants) {
                    state=ImageState {identity:Some((self.source.clone(),self.variants.clone())),..Default::default()};
                }
...
impl ManagedImageSource for ImageProvider {
    fn intrinsic_size(&self,window:&mut Window,cx:&mut App)->Option<Result<Size<Pixels>,ImageCacheError>> {
        self.with_state(window,cx,|state,window,cx| {
            if let Some(error)=state.variant.as_ref().and_then(|variant|variant.read(cx).error.clone()) {return Some(Err(error));}
            // Explicit candidates provide metadata without downloading the original.
            if let Some((_,size))=self.variants.iter().max_by_key(|(_,size)|u64::from(size.0)*u64::from(size.1)) {
                if let Some(Err(error))=state.source.as_ref().and_then(|source|source.read(cx).result.clone()) {return Some(Err(error));}
                return Some(Ok(gpui::size(gpui::px(size.0 as f32),gpui::px(size.1 as f32))));
            }
            if let Some(size)=state.intrinsic {
                return Some(Ok(gpui::size(gpui::px(size.0 as f32),gpui::px(size.1 as f32))));
            }
            if state.source.as_ref().is_none_or(|source|source.read(cx).key!=self.source) {
                state.variant=None;
                state.source=Some(ImageData::acquire(&self.source,cx));
            }
            let result=state.source.as_ref().unwrap().update(cx,|source,_|source.get(window.current_view()));
            if let Some(Ok(encoded))=&result {state.intrinsic=Some(encoded.size);}
            result.map(|result|result.map(|encoded|gpui::size(gpui::px(encoded.size.0 as f32),gpui::px(encoded.size.1 as f32))))
        })
    }

    fn image(&self,request:ImageRequest,window:&mut Window,cx:&mut App)->Option<Result<ManagedImageFrame,ImageCacheError>> {
        self.with_state(window,cx,|state,window,cx| {
            if !request.visible {
                state.variant=None;
                state.previous=None;
            ...
            if !self.variants.is_empty() {
                let intrinsic=self.variants.iter().max_by_key(|(_,size)|u64::from(size.0)*u64::from(size.1)).unwrap().1;
                let target=target_size(intrinsic,request);
                let selected=self.variants.iter().filter(|(_,size)|size.0>=target.0 && size.1>=target.1)
                    .min_by_key(|(_,size)|u64::from(size.0)*u64::from(size.1))
                    .or_else(||self.variants.iter().max_by_key(|(_,size)|u64::from(size.0)*u64::from(size.1))).unwrap();
                if state.source.as_ref().is_none_or(|source|source.read(cx).key!=selected.0) {
                    state.variant=None;
                    state.source=Some(ImageData::acquire(&selected.0,cx));
                }
            }
            if state.source.is_none() {state.source=Some(ImageData::acquire(&self.source,cx));}
            let source=state.source.as_ref()?;
            let encoded=match source.update(cx,|source,_|source.get(window.current_view()))? {
                Ok(encoded)=>encoded,Err(error)=>return Some(Err(error)),
            };
            let mut target=target_size(encoded.size,request);
            if !encoded.scalable {target=(target.0.min(encoded.size.0),target.1.min(encoded.size.1));}
            if state.variant.as_ref().is_none_or(|variant|variant.read(cx).target!=target) {
                state.previous=state.variant.as_ref().and_then(|variant|variant.read(cx).current.clone());
                state.variant=Some(ImageVariant::acquire(source.clone(),encoded,target,cx));
            }
            let frame=state.variant.as_ref().unwrap().update(cx,|variant,cx|variant.frame(window.current_view(),request.animate,cx));
            if frame.is_some() {state.previous=None;}
...
fn image_source(source:&str)->ImageSource {
    ImageSource::Managed(Arc::new(ImageProvider{source:ImageKey::parse(source),variants:Vec::new()}))
}
...
pub(super) fn render(
    ...
    let source = ImageSource::Managed(Arc::new(ImageProvider {
        ...
        variants: image.sources.iter().map(|candidate| (ImageKey::parse(&candidate.source), (candidate.width, candidate.height))).collect(),
    }));
...
mod tests {
    ...
    use gpui::{AppContext, Asset, ImageAssetLoader, Resource, RenderImage, WeakEntity};
    use resource::Images;
    use std::io::Cursor;
    ...
    fn loaded_pixels(source:&str,cx:&App)->Arc<RenderImage> {
        let source=cx.global::<Images>().sources[&ImageKey::parse(source)].upgrade().unwrap();
        let variant=cx.global::<Images>().variants.iter().find(|(key,_)|key.0==source.entity_id()).unwrap().1.upgrade().unwrap();
        variant.read(cx).current.as_ref().unwrap().image.clone()
    }

    #[test]
    fn decode_target_respects_fit_dpi_and_resize_buckets() {
        let mut request=ImageRequest {bounds:gpui::Bounds::new(gpui::point(gpui::px(0.),gpui::px(0.)),gpui::size(gpui::px(200.),gpui::px(150.))),object_fit:ObjectFit::Contain,scale_factor:2.,animate:false,visible:true};
        let contain=target_size((4000,3000),request);
        assert!(contain.0>=400 && contain.0<480 && contain.1>=300 && contain.1<360);
        request.bounds.size.width=gpui::px(201.);
        assert_eq!(target_size((4000,3000),request),contain);
        request.bounds.size.height=gpui::px(200.);
        request.object_fit=ObjectFit::Cover;
        let cover=target_size((4000,3000),request);
        assert!(cover.0>=534 && cover.1>=400, "cover must retain pixels beyond the crop");
        request.scale_factor=1.;
        let low_dpi=target_size((4000,3000),request);
    ...
    fn source_set_loads_only_the_smallest_sufficient_variant(cx:&mut gpui::TestAppContext) {
        let window=cx.open_window(gpui::size(gpui::px(200.),gpui::px(200.)),|_,_|SolidRoot::new(crate::InMemoryAdapter::new()));
        ...
        window.update(cx,|root,window,cx| {
            ...
            node.style=Some(Style{width:Some(100.),height:Some(100.),..Default::default()});
            node.host_properties=Some(HostProperties::Image(crate::ImageProperties {
                source:"/must-not-load-original.png".into(),object_fit:2,fallback_source:None,
                sources:vec![crate::ImageCandidate{source:svg.into(),width:200,height:200},crate::ImageCandidate{source:"/must-not-load-large.png".into(),width:4000,height:4000}],
            }));
            root.apply_decoded_message_in_window(crate::protocol::DecodedMessage::Snapshot(crate::Snapshot::new(1,1,0,1,vec![crate::Node::new(1,0,0,crate::tree::KIND_VIEW),node])),window,cx).unwrap();
        }).unwrap();
        draw(cx,window);
        window.update(cx,|_,window,cx| {
            let pixels=loaded_pixels(svg,cx);
            assert!(pixels.size(0).width.0<=200 && pixels.size(0).height.0<=200);
            assert!(window.has_image_atlas_entry(&pixels));
            assert!(!cx.global::<Images>().sources.contains_key(&ImageKey::parse("/must-not-load-original.png")));
            assert!(!cx.global::<Images>().sources.contains_key(&ImageKey::parse("/must-not-load-large.png")));
        }).unwrap();
    }
    ...
    fn publish(cx: &mut gpui::TestAppContext, window: gpui::WindowHandle<SolidRoot>, source: Option<&str>, fallback: Option<&str>) {
        window.update(cx, |root, window, cx| {
            ...
            if let Some(source) = source {
                ...
                node.style = Some(Style { width: Some(100.), height: Some(100.), ..Default::default() });
                node.host_properties = Some(HostProperties::Image(crate::ImageProperties {
                    source: source.into(), object_fit: 2, fallback_source: fallback.map(Into::into), sources: Vec::new(),
                }));
                nodes.push(node);
            }
            let revision = root.store().revision();
            let snapshot = crate::Snapshot::new(1, 1, revision, revision + 1, nodes);
            root.apply_decoded_message_in_window(crate::protocol::DecodedMessage::Snapshot(snapshot), window, cx).unwrap();
        }).unwrap();
        draw(cx, window);
    }
    ...
    fn images_share_across_windows_and_release_pixels_and_atlases(cx: &mut gpui::TestAppContext) {
        let first = cx.open_window(gpui::size(gpui::px(200.), gpui::px(200.)), |_, _| SolidRoot::new(crate::InMemoryAdapter::new()));
        let second = cx.open_window(gpui::size(gpui::px(200.), gpui::px(200.)), |_, _| SolidRoot::new(crate::InMemoryAdapter::new()));
        ...
        for window in [first, second] {
            window.update(cx, |_, window, _| assert!(window.has_image_atlas_entry(&pixels))).unwrap();
        }
        publish(cx, first, None, None);
        assert!(lease.upgrade().is_some(), "another window still owns the image");
        second.update(cx, |_, window, _| window.remove_window()).unwrap();
        cx.run_until_parked();
        draw(cx, first);
        assert!(lease.upgrade().is_none(), "closing the last image window releases the load");
        first.update(cx, |_, window, cx| {
            assert!(!window.has_image_atlas_entry(&pixels), "all windows must release old atlas tiles");
            assert!(cx.global::<Images>().sources.is_empty());
        }).unwrap();
        let weak_pixels = Arc::downgrade(&pixels);
        drop(pixels);
        assert!(weak_pixels.upgrade().is_none(), "decoded pixels must not enter the global asset cache");
    }
    ...
    fn replacing_primary_releases_fallback_without_eager_loading(cx: &mut gpui::TestAppContext) {
        ...
        let window = cx.open_window(gpui::size(gpui::px(200.), gpui::px(200.)), |_, _| SolidRoot::new(crate::InMemoryAdapter::new()));
        publish(cx, window, Some("https://images.example/missing.png"), Some(INLINE_PNG));
        let old = cx.update(|cx| {
            Arc::downgrade(&loaded_pixels(INLINE_PNG,cx))
        });
        ...
        assert!(old.upgrade().is_none(), "an unused fallback must release its decoded image");
    ...
    fn removing_last_image_cancels_pending_http_load(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|cx| cx.set_http_client(gpui::http_client::FakeHttpClient::create(move |_| {
            let receiver = receiver.try_lock().expect("uncontended test receiver").take().expect("one shared load");
            async move {
                let _ = receiver.await;
                Ok(gpui::http_client::Response::builder().status(404).body(Vec::new().into()).unwrap())
            }
        })));
        let window = cx.open_window(gpui::size(gpui::px(200.), gpui::px(200.)), |_, _| SolidRoot::new(crate::InMemoryAdapter::new()));
        ...
        publish(cx, window, None, None);
        assert!(sender.is_canceled(), "a detached image must not leave a download running");
    }
