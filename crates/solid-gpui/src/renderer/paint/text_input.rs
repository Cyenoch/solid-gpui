...
use gpui::{
    ...
    StatefulInteractiveElement, Styled, StyledText, TextRun, Window, div, fill, point, px,
    relative, rgba, size,
};
...
impl Element for TextInputElement {
    ...
    fn paint(
        ...
        if !self.disabled && self.focus.is_focused(window) {
            self.entity
...
fn text_input_text_color(text_style: &gpui::TextStyle, is_placeholder: bool) -> gpui::Hsla {
    if is_placeholder {
...
pub(super) fn render_text_input(
    ...
    if node.listener_id != 0 && !input_disabled {
    ...
    input_element = input_element.on_mouse_move(move |event, window, app| {
        if input_disabled || !event.dragging() {
            return;
        }
        mouse_entity.update(app, |root, cx| {
...
pub(super) fn render_rich_text(
    ...
    for (run_node_id, run_listener_id) in targets {
        ...
        let focus_element = div()
            ...
            .accessibility_id(format!("solid-gpui-text-run-{run_node_id}"))
...
mod text_input_style_tests {
    ...
    fn placeholder_remains_readable_on_light_and_dark_inputs() {
        ...
        for (foreground, background) in [(0x252a26, 0xffffff), (0xededeb, 0x1a1c1e)] {
            let style = gpui::TextStyle {
                color: gpui::rgb(foreground).into(),
                ..Default::default()
            };
            let background = gpui::rgb(background);
            let placeholder = text_input_text_color(&style, true);
            let text_luminance = luminance(background.blend(placeholder.to_rgb()));
            let background_luminance = luminance(background);
            let contrast = (text_luminance.max(background_luminance) + 0.05)
                / (text_luminance.min(background_luminance) + 0.05);
            assert!(
                contrast >= 4.5,
                "placeholder contrast must remain readable: {contrast}"
            );
            assert_eq!(text_input_text_color(&style, false), style.color);
        }
    }
}
