...
impl SolidRoot {
    ...
    pub(super) fn text_input_enabled(&self, node_id: u32) -> bool {
        self.store.get(node_id).is_some_and(|node| {
            matches!(node.host_properties.as_ref(), Some(HostProperties::TextInput(input)) if !input.disabled)
        })
    }

    pub(super) fn reconcile_disabled_input_focus(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let active_disabled = self.active_input.filter(|id| !self.text_input_enabled(*id));
        let focused_disabled = self.input_states.keys().copied().find(|id| {
            !self.text_input_enabled(*id)
                && self
                    .focus_handles
                    .get(id)
                    .is_some_and(|handle| handle.is_focused(window))
        });
        if let Some(node_id) = active_disabled.or(focused_disabled) {
            if self
                .focus_handles
                .get(&node_id)
                .is_some_and(|handle| handle.is_focused(window))
            ...
            self.set_input_focus(node_id, false);
        ...
        if let Some(ids) = affected {
            for id in ids {
                ...
                let keeps_focus_handle = self.store.get(*id).is_some_and(|node| {
                    self.input_states.contains_key(id) || focus_handle_required(node)
                });
            ...
            self.focus_handles.retain(|id, _| {
                self.input_states.contains_key(id)
                    || self.store.get(*id).is_some_and(focus_handle_required)
            });
        }
            ...
            if let Some(HostProperties::TextInput(input)) = node.host_properties.clone() {
                ...
                *focus_handle = focus_handle.clone().tab_stop(!input.disabled);
                ...
                if input.disabled {
                    state.focused = false;
                    state.marked = None;
                    state.history.composition_before = None;
                }
                state.set_max_length(input.max_length.map(|value| value as usize));
                state.apply_controlled(&input);
                if input.disabled && self.active_input == Some(id) {
                    self.active_input = None;
                    self.text_input_drag_anchor = None;
                }
                if input.disabled && was_focused {
                    self.emit_input_event(id, EVENT_BLUR);
                }
            } else if focus_handle_required(node) {
                let focus_handle = self
                    .focus_handles
                    .entry(id)
                    .or_insert_with(|| cx.focus_handle());
    ...
    pub(super) fn emit_submit_event(&self, node_id: u32) {
        ...
        if input.disabled || input.multiline || node.listener_id == 0 {
            return;
        }
    ...
    pub(super) fn set_input_focus(&mut self, node_id: u32, focused: bool) {
        if focused && !self.text_input_enabled(node_id) {
            return;
        }
        if focused {
    ...
    fn text_input_index_for_point(
        ...
        if !self.text_input_enabled(node_id) {
            return None;
        }
        self.active_input = Some(node_id);
        let fallback = self
...
impl SolidRoot {
    fn active_input_state(&self) -> Option<&NativeInputState> {
        self.active_input
            .filter(|id| self.text_input_enabled(*id))
            .and_then(|id| self.input_states.get(&id))
    }

    fn active_input_state_mut(&mut self) -> Option<&mut NativeInputState> {
        let id = self.active_input?;
        if !self.text_input_enabled(id) {
            return None;
        }
        self.input_states.get_mut(&id)
    }
    fn emit_input_change_and_selection(&self) {
        if let Some(id) = self.active_input.filter(|id| self.text_input_enabled(*id)) {
    ...
    pub(super) fn replace_text_input(
        ...
        if !self.text_input_enabled(node_id) || !self.input_states.contains_key(&node_id) {
            return false;
        }
        self.active_input = Some(node_id);
        if let Some(state) = self.input_states.get_mut(&node_id) {
...
impl EntityInputHandler for SolidRoot {
    fn text_for_range(
        &mut self,
        range: Range<usize>,
    ...
    fn character_index_for_point(
        ...
        let utf8_offset = layout
            .text
            .closest_index_for_point(local + layout.scroll_offset);
...
mod tests {
    ...
    fn selectable_selection_clamps_changed_text_to_utf8_boundaries() {
        let mut range = 1..8;
        SolidRoot::clamp_utf8_range(&mut range, "a😀", "a😀".len());
        assert_eq!(range, 1..5);
        let mut reversed = Range { start: 8, end: 1 };
        SolidRoot::clamp_utf8_range(&mut reversed, "a😀", "a😀".len());
