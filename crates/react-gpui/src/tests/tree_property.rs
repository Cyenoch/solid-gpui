...
use crate::tree::{NodeStore, StoredNode};
...
fn nested_style_compatible(style: Option<&Style>) -> bool {
    ...
    style.width.is_none()
        ...
        && style.flex_direction.is_none_or(|value| value == 0)
        ...
        && style.position.is_none_or(|value| value == 0)
        && style.left.is_none()
        && style.top.is_none()
        && style.right.is_none()
...
fn assert_renderer_side_maps(root: &ReactRoot) -> Result<(), String> {
...
fn cache_ids(root: &ReactRoot) -> HashSet<u32> {
