...
fn peer_path() -> String {
    std::env::var("CARGO_BIN_EXE_react-gpui-frame-peer")
        .or_else(|_| std::env::var("CARGO_BIN_EXE_react_gpui_frame_peer"))
...
fn process_adapter_round_trips_snapshot_and_event_frames() {
    let peer = peer_path();
    let command = Command::new(peer);
    let runtime = ProcessAdapter::spawn(command).expect("spawn frame peer");
    ...
    assert_eq!(event.event_type, EVENT_PRESS);
...
fn process_adapter_round_trips_actual_bun_counter() {
    ...
    command
        ...
        .args(["run", "packages/react-gpui/examples/counter.tsx"]);
    ...
    assert!(patch.operations.iter().any(|operation| {
        matches!(
            operation,
            react_gpui::PatchOperation::Update {
                mask,
                text: Some(text),
                ..
            } if mask & react_gpui::UPDATE_TEXT != 0 && text == "1"
        )
    }));
