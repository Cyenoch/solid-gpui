...
This follows from actual GPUI behavior: [explicit ids build the GlobalElementId stack](https://docs.rs/crate/gpui-pre/0.3.3/source/src/element.rs#300); [element state is retrieved by GlobalElementId and state type](https://docs.rs/crate/gpui-pre/0.3.3/source/src/window.rs#3976); the animation element retains its start time through the same state system. [TextSelectionScopeMarker forwards the child id](../../vendor/gpui-kit/crates/base/src/text_selection.rs#L1722), so selection scope was not an identity boundary.

**Current code:** [OverlayElement](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L122) adds an `("overlay", lifetime id)` boundary and forwards the child's own layout, prepaint, and paint. Root uses it for the entire Dialog, including the outer animation, while Sheet's containing Div receives the lifetime id ([Dialog wrapper](../../vendor/gpui-kit/crates/component/src/root.rs#L292), [Sheet wrapper](../../vendor/gpui-kit/crates/component/src/root.rs#L226)). The Dialog popup, base host, and focus trap now use constant ids within that lifetime boundary ([popup](../../vendor/gpui-kit/crates/component/src/dialog/dialog.rs#L571), [base host](../../vendor/gpui-kit/crates/base/src/dialog.rs#L496), [focus trap](../../vendor/gpui-kit/crates/base/src/dialog.rs#L504)). Therefore moving B in the stack does not change those state paths; index remains available for z-order, topmost state, and visual offset.

Both builder invocations also run in the lifetime namespace, covering eager `window.use_keyed_state` calls before the returned elements render ([Sheet builder](../../vendor/gpui-kit/crates/component/src/root.rs#L215), [Dialog builder](../../vendor/gpui-kit/crates/component/src/root.rs#L259)). This places the boundary in the overlay layer instead of adding wrappers to individual controls. Runtime verification must still cover both eager builder state and actual rendered child state.
...
**Current code:** [Sheet consumes pending focus only if no Dialog survives](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L272). With A alive, the pending task remains intact while the first Dialog's eventual predecessor is changed to the new Sheet. [The restore helper](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L184) replaces older tasks, and opening a new Dialog still deliberately consumes pending focus so its old timer cannot steal focus from the newly opened top layer. These branches are statically coherent; delayed native focus and Tab assertions remain necessary.
...
The current Sheet checks the cloned lifetime after the builder and publishes its prepaint size only while the id still matches the current Sheet ([post-builder lifetime check](../../vendor/gpui-kit/crates/component/src/root.rs#L216), [prepaint ownership check](../../vendor/gpui-kit/crates/component/src/root.rs#L230)). Dialog snapshots are retained by live lifetime before surviving indexes and dimming placement are computed ([survivor filtering](../../vendor/gpui-kit/crates/component/src/root.rs#L273)). This avoids leaving the only visible overlay on a retired snapshot or leaving B at A-and-B's former top index after A disappears. An adversarial native builder case should verify these boundaries.

## Behaviors supported by the current source

| Scenario | Source reasoning |
| --- | --- |
| Close A while newer B survives | [Close lookup](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L231) removes A by id and reconnects the next Dialog's previous-focus link. It does not pop B. |
| Close the same token or a clone again | Its id no longer resolves. [Lifetime retirement](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L110) also flips the shared open flag only once. |
...
| Dialog decision callback opens B while closing A | Base invokes the decision first; the component close request captures A's lifetime id, preserving new B ([base close ordering](../../vendor/gpui-kit/crates/base/src/dialog.rs#L507), [owned request](../../vendor/gpui-kit/crates/component/src/dialog/dialog.rs#L562)). |
| Sheet surface callback opens a replacement | Base requests close before invoking `on_close`, and the component request captures the old lifetime id ([base ordering](../../vendor/gpui-kit/crates/base/src/sheet.rs#L16), [owned Sheet request](../../vendor/gpui-kit/crates/component/src/sheet.rs#L255)). |
...
| Close all current Dialogs, callbacks reopen | [The synchronous close loop](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L262) removes the present stack; deferred callbacks may create a new stack afterward. This is coherent callback behavior. |
| Root/window is destroyed | [Token openness](../../vendor/gpui-kit/crates/component/src/root/overlay.rs#L151) also requires its weak Root to upgrade. No window-teardown `on_closed` guarantee is documented or assumed here. |
